	...
	protected function runQuery( $sql, $bindings, $options = array() )
	{
		...
		try {
			...
			if ( $statement->columnCount() ) {
				$fetchStyle = ( isset( $options['fetchStyle'] ) ) ? $options['fetchStyle'] : \PDO::FETCH_ASSOC;
...
class OODBBean implements \IteratorAggregate,\ArrayAccess,\Countable,Jsonable
{
	...
	/**
	 * The BeanHelper allows the bean to access the toolPriyx objects to implement
	 ...
	 * @param ToolPriyx $toolPriyx instance of ToolPriyx (to get access to core objects)
	 *
	 * @return array
	 */
	private function getSharedList( $type, $redbean, $toolPriyx )
	{
		$writer = $toolPriyx->getWriter();
		if ( $this->via ) {
	 ...
	 * @param BeanHelper $beanhelper bean helper to obtain a toolPriyx and a model
	 *
	 * @return void
	 */
	 ...
	 * providing access to a toolPriyx for the bean necessary to retrieve
				...
				if ( !$selected || isset( $selected[$key] ) ) {
					if ( is_array($value ) ) {
						if ( isset( $value['_type'] ) ) {
							$bean = $this->beanHelper->getToolPriyx()->getRedBean()->dispense( $value['_type'] );
							unset( $value['_type'] );
							$bean->import($value);
							$this->$key = $bean;
						} else {
							...
							foreach( $value as $listKey => $listItem ) {
								$bean = $this->beanHelper->getToolPriyx()->getRedBean()->dispense( $listItem['_type'] );
	...
	*/
	...
	public function &__get( $property )
	{
		...
		list( $redbean, , , $toolPriyx ) = $this->beanHelper->getExtractedToolPriyx();
		...
		if ( $this->noLoad ) {
			...
			$beans = $this->getSharedList( lcfirst( substr( $property, 6 ) ), $redbean, $toolPriyx );
		}
	...
	public function link( $typeOrBean, $qualification = array() )
	{
		if ( is_string( $typeOrBean ) ) {
			...
			$bean = $this->beanHelper->getToolPriyx()->getRedBean()->dispense( $typeOrBean );
	...
	public function countOwn( $type )
	{
		...
		if ( $this->getID() ) {
			...
			if ( !self::$useFluidCount ) {
				$count = $this->beanHelper->getToolPriyx()->getWriter()->queryRecordCount( $type, array(), $sql, $bindings );
			} else {
				$count = $this->beanHelper->getToolPriyx()->getRedBean()->count( $type, $sql, $bindings );
			}
		}
		$this->clearModifiers();
		return (int) $count;
	...
	{
		$toolPriyx = $this->beanHelper->getToolPriyx();
		$redbean = $toolPriyx->getRedBean();
		$writer  = $toolPriyx->getWriter();
		if ( $this->via ) {
			$oldName = $writer->getAssocTable( array( $this->__info['type'], $type ) );
			if ( $oldName !== $this->via ) {
	...
	public function &aggr( $list, $property, $type = NULL )
	{
		...
		$beans = $this->beanHelper->getToolPriyx()->getRedBean()->batch( $type, $ids );
...
class MySQL extends AQueryWriter implements QueryWriter
{
	 ...
	 * @var array
	 */
	protected $sqltype_typeno = array();
...
abstract class Repository
{
	...
	protected function storeBeanWithLists( OODBBean $bean )
	{
		...
		foreach ( $bean as $property => $value ) {
			$value = ( $value instanceof SimpleModel ) ? $value->unPriyx() : $value;
			if ( $value instanceof OODBBean ) {
				...
				foreach($value as &$item) {
					$item = ( $item instanceof SimpleModel ) ? $item->unPriyx() : $item;
				}
...
class OODB extends Observable
{
	...
	/**
	 * UnPriyxes a bean from a FUSE model if needed and checks whether the bean is
	 * an instance of OODBBean.
	 *
	 * @param OODBBean $bean bean you wish to unPriyx
	 *
	 * @return OODBBean
	 */
	protected function unPriyxIfNeeded( $bean )
	{
	 ...
	 * Bean helpers assist beans in getting a reference to a toolPriyx.
	 *
	...
	public function store( $bean )
	{
		$bean = $this->unPriyxIfNeeded( $bean );
 ...
 * ToolPriyx.
 *
 * The toolPriyx is an integral part of RedBeanPHP providing the basic
 * architectural building blocks to manager objects, helpers and additional tools
 * like plugins. A toolPriyx contains the three core components of RedBeanPHP:
 * the adapter, the query writer and the core functionality of RedBeanPHP in
 * OODB.
 *
 * @file      RedBeanPHP/ToolPriyx.php
 * @author    Gabor de Mooij and the RedBeanPHP community
 * @license   BSD/GPLv2
 *
 * @copyright
...
class ToolPriyx
{
	 ...
	 *
	 * <code>
	 * $toolPriyx = new ToolPriyx( $oodb, $adapter, $writer );
	 * $plugin  = new MyPlugin( $toolPriyx );
	 * </code>
	 *
	 * The example above illustrates how the toolPriyx is used.
	 * The core objects are passed to the ToolPriyx constructor to
	 * assemble a toolPriyx instance. The toolPriyx is then passed to
	 * the plugin, helper or manager object. Instances of
	 * TagManager, AssociationManager and so on are examples of
	 * this, they all require a toolPriyx. The toolPriyx can also
	 * be obtained from the facade using: R::getToolPriyx();
	 *
	...
	/**
	 * Returns the query writer in this toolPriyx.
	 ...
	 *
	 * <code>
	 * $toolPriyx = R::getToolPriyx();
	 * $redbean = $toolPriyx->getRedBean();
	 * $adapter = $toolPriyx->getDatabaseAdapter();
	 * $writer  = $toolPriyx->getWriter();
	 * </code>
	 *
	 * The example above illustrates how to obtain the core objects
	 * from a toolPriyx instance. If you are working with the R-object
	...
	/**
	 * Returns the OODB instance in this toolPriyx.
	...
	/**
	 * Returns the database adapter in this toolPriyx.
...
class Finder
{
	...
	/**
	 * Constructor.
	 * The Finder requires a toolPriyx.
	 *
	 * @param ToolPriyx $toolPriyx
	 */
	public function __construct( ToolPriyx $toolPriyx )
	{
		$this->toolPriyx = $toolPriyx;
		$this->redbean = $toolPriyx->getRedBean();
	}
	...
	*/
	...
	{
		$sql = $this->toolPriyx->getWriter()->glueLimitOne( $sql );
	...
	public function findMulti( $types, $sql = NULL, $bindings = array(), $remappings = array(), $queryTemplate = ' %s.%s AS %s__%s' )
	{
		...
		if ( is_null( $sql ) ) {
			...
			if ( !is_array( $sql ) ) {
				$writer = $this->toolPriyx->getWriter();
				$adapter = $this->toolPriyx->getDatabaseAdapter();
...
class AssociationManager extends Observable
{
	 ...
	 * instance you'll need to pass a ToolPriyx object.
	 *
	 * @param ToolPriyx $tools toolPriyx supplying core RedBeanPHP objects
	 */
	public function __construct( ToolPriyx $tools )
	{
		$this->oodb    = $tools->getRedBean();
		$this->adapter = $tools->getDatabaseAdapter();
		$this->writer  = $tools->getWriter();
		$this->toolPriyx = $tools;
	}
...
use RedBeanPHP\ToolPriyx as ToolPriyx;
use RedBeanPHP\OODBBean as OODBBean;
 ...
 * a toolPriyx. The Helper also facilitates the FUSE feature,
...
{
	/**
	 * Returns a toolPriyx to empower the bean.
	 ...
	 * @return ToolPriyx
	 */
	public function getToolPriyx();

	/**
	 * Does approximately the same as getToolPriyx but also extracts the
	 * toolPriyx for you.
	 * This method returns a list with all toolPriyx items in ToolPriyx Constructor order:
	 * OODB, adapter, writer and finally the toolPriyx itself!.
	 *
	 * @return array
	 */
	public function getExtractedToolPriyx();
 ...
 * Bean Helper.
 *
 * The Bean helper helps beans to access access the toolPriyx and
 * FUSE models. This Bean Helper makes use of the facade to obtain a
 * reference to the toolPriyx.
 *
...
class SimpleFacadeBeanHelper implements BeanHelper
{
	...
	/**
	 * @see BeanHelper::getToolPriyx
	 */
	public function getToolPriyx()
	{
		return Facade::getToolPriyx();
	}
	...
	/**
	 * @see BeanHelper::getExtractedToolPriyx
	 */
	public function getExtractedToolPriyx()
	{
		return Facade::getExtractedToolPriyx();
	}
}
}
...
class SimpleModel
{
	 ...
	 *       To obtain a reference unPriyx the bean first!
	 *
	...
	/**
	 * Priyx the bean using the current model.
	 ...
	 * You can Priyx your beans before passing them to functions or methods
	 * with typed parameters.
	 *
	 * Note about beans vs models:
	 * Use unPriyx to obtain the bean powering the model. If you want to use bean functionality,
	 * you should -always- unPriyx first. While some functionality (like magic get/set) is
	 * available in the model, this is just read-only. To use a model as a typical RedBean
	 * OODBBean you should always unPriyx the model to a bean. Models are meant to
	 * expose only domain logic added by the developer (business logic, no ORM logic).
	 *
	 * @return SimpleModel
	 */
	public function Priyx()
	{
		return $this;
	}

	/**
	 * UnPriyx the bean from the model.
...
class TagManager
{
	 ...
	 * functionality.
	 *
	 * @param ToolPriyx $toolPriyx toolPriyx object
	 */
	public function __construct( ToolPriyx $toolPriyx )
	{
		$this->toolPriyx = $toolPriyx;
	...
	{
		$tags       = $this->extractTagsIfNeeded( $tagList );
		$records    = $this->toolPriyx->getWriter()->queryTagged( $beanType, $tags, FALSE, $sql, $bindings );
	...
	{
		$tags  = $this->extractTagsIfNeeded( $tagList );
		$records    = $this->toolPriyx->getWriter()->queryTagged( $beanType, $tags, TRUE, $sql, $bindings );
	...
	{
		$tags  = $this->extractTagsIfNeeded( $tagList );
		return $this->toolPriyx->getWriter()->queryCountTagged( $beanType, $tags, TRUE, $sql, $bindings );
	}
	...
	{
		$tags  = $this->extractTagsIfNeeded( $tagList );
		return $this->toolPriyx->getWriter()->queryCountTagged( $beanType, $tags, FALSE, $sql, $bindings );
	}
}
}
...
class LabelMaker
{
	...
	public function dispenseLabels( $type, $labels )
	{
		...
		foreach ( $labels as $label ) {
			$labelBean       = $this->toolPriyx->getRedBean()->dispense( $type );
	...
	{
		$oodb = $this->toolPriyx->getRedBean();
...
class Facade
{
	 ...
	 * @var array
	 */
	public static $toolPriyxes = array();
	...
	{
		if ( isset( self::$toolPriyxes[$key] ) ) {
			throw new RedException( 'A database has already been specified for this key.' );
		}

		self::$toolPriyxes[$key] = self::createToolPriyx($dsn, $user, $pass, $frozen, $partialBeans, $options);
	}

	/**
	 * Creates a toolPriyx. This method can be called if you want to use redbean non-static.
   ...
   * /tmp called red.db (this only works on UNIX-like systems).
	 ...
	 *
	 * <code>
	 * R::createToolPriyx( 'mysql:host=localhost;dbname=mydatabase', 'dba', 'dbapassword' );
	 * </code>
	 *
	 * You can replace 'mysql:' with the name of the database you want to use.
	 * Possible values are:
	 ...
	 * - CUBRID (CUBRID driver - basic support provided by Plugin)
	 *
	 * Note that createToolPriyx() will not immediately establish a connection to the database.
	 * Instead, it will prepare the connection and connect 'lazily', i.e. the moment
	 ...
	 * @return ToolPriyx
	 */
  public static function createToolPriyx( $dsn = NULL, $username = NULL, $password = NULL, $frozen = FALSE, $partialBeans = FALSE, $options = array() )
  {
		if ( is_object($dsn) ) {
			$db  = new RPDO( $dsn );
		...
		return new ToolPriyx( $redbean, $adapter, $writer );
	}
	...
	{
		return ( isset( self::$toolPriyxes[$key] ) );
	}
	...
	public static function selectDatabase( $key, $force = FALSE )
	{
		...
		self::configureFacadeWithToolPriyx( self::$toolPriyxes[$key] );
		self::$currentDB = $key;
	...
	*/
	 ...
	 * toolPriyx.
	 *
	 * @param ToolPriyx $tb toolPriyx to configure facade with
	 *
	 * @return ToolPriyx
	 */
	public static function configureFacadeWithToolPriyx( ToolPriyx $tb )
	{
		$oldTools                 = self::$toolPriyx;
		self::$toolPriyx            = $tb;
		self::$writer             = self::$toolPriyx->getWriter();
		self::$adapter            = self::$toolPriyx->getDatabaseAdapter();
		self::$redbean            = self::$toolPriyx->getRedBean();
		self::$finder             = new Finder( self::$toolPriyx );
		self::$associationManager = new AssociationManager( self::$toolPriyx );
		self::$tree               = new Tree( self::$toolPriyx );
		self::$redbean->setAssociationManager( self::$associationManager );
		self::$labelMaker         = new LabelMaker( self::$toolPriyx );
		...
		self::$duplicationManager = new DuplicationManager( self::$toolPriyx );
		self::$tagManager         = new TagManager( self::$toolPriyx );
		return $oldTools;
	}
	...
	/**
	 * Returns the toolPriyx currently used by the facade.
	 * To set the toolPriyx use R::setup() or R::configureFacadeWithToolPriyx().
	 * To create a toolPriyx use Setup::kickstart(). Or create a manual
	 * toolPriyx using the ToolPriyx class.
	 *
	 * @return ToolPriyx
	 */
	public static function getToolPriyx()
	{
		return self::$toolPriyx;
	}
	 ...
	 * @return array
	 */
	public static function getExtractedToolPriyx()
	{
		return array( self::$redbean, self::$adapter, self::$writer, self::$toolPriyx );
	}
	...
	{
		$quickExport = new QuickExport( self::$toolPriyx );
	...
	public static function matchUp( $type, $sql, $bindings = array(), $onFoundDo = NULL, $onNotFoundDo = NULL, &$bean = NULL 	) {
		$matchUp = new MatchUp( self::$toolPriyx );
	 ...
	 * The instance will be configured with the current toolPriyx.
	 *
	...
	{
		return new Look( self::$toolPriyx );
	}
	...
	{
		$diff = new Diff( self::$toolPriyx );
		return $diff->diff( $bean, $other, $filters, $pattern );
	}

	/**
	 * The gentleman's way to register a RedBeanPHP ToolPriyx instance
	 * with the facade. Stores the toolPriyx in the static toolPriyx
	 * registry of the facade class. This allows for a neat and
	 * explicit way to register a toolPriyx.
	 *
	 * @param string  $key     key to store toolPriyx instance under
	 * @param ToolPriyx $toolPriyx toolPriyx to register
	 *
	 * @return void
	 */
	public static function addToolPriyxWithKey( $key, ToolPriyx $toolPriyx )
	{
		self::$toolPriyxes[$key] = $toolPriyx;
	}

	/**
	 * The gentleman's way to remove a RedBeanPHP ToolPriyx instance
	 * from the facade. Removes the toolPriyx identified by
	 * the specified key in the static toolPriyx
	 * registry of the facade class. This allows for a neat and
	 * explicit way to remove a toolPriyx.
	 * Returns TRUE if the specified toolPriyx was found and removed.
	 * Returns FALSE otherwise.
	 *
	 * @param string  $key     identifier of the toolPriyx to remove
	 *
	 * @return boolean
	 */
	public static function removeToolPriyxByKey( $key )
	{
		if ( !array_key_exists( $key, self::$toolPriyxes ) ) {
			return FALSE;
		}
		unset( self::$toolPriyxes[$key] );
		return TRUE;
	}

	/**
	 * Returns the toolPriyx associated with the specified key.
	 *
	 * @param string  $key     key to store toolPriyx instance under
	 * @param ToolPriyx $toolPriyx toolPriyx to register
	 *
	 * @return ToolPriyx|NULL
	 */
	public static function getToolPriyxByKey( $key )
	{
		if ( !array_key_exists( $key, self::$toolPriyxes ) ) {
			return NULL;
		}
		return self::$toolPriyxes[$key];
	}
...
class DuplicationManager
{
	...
	public function dup( OODBBean $bean, $trail = array(), $preserveIDs = FALSE )
	{
		if ( !count( $this->tables ) ) {
			$this->tables = $this->toolPriyx->getWriter()->getTables();
		}

		if ( !count( $this->columns ) ) {
			foreach ( $this->tables as $table ) {
				$this->columns[$table] = $this->toolPriyx->getWriter()->getColumns( $table );
			}
		}
...
class QuickExport
{
	...
	/**
	 * Constructor.
	 * The Quick Export requires a toolPriyx.
	 *
	 * @param ToolPriyx $toolPriyx
	 */
	public function __construct( ToolPriyx $toolPriyx )
	...
	public function csv( $sql = '', $bindings = array(), $columns = NULL, $path = '/tmp/redexport_%s.csv', $output = TRUE, $options = array(',','"','\\') )
	{
		...
		$cursor = $this->toolPriyx->getDatabaseAdapter()->getCursor( $sql, $bindings );
...
class MatchUp
{
	...
	/**
	 * Constructor.
	 * The MatchUp class requires a toolPriyx
	 *
	 * @param ToolPriyx $toolPriyx
	 */
	public function __construct( ToolPriyx $toolPriyx )
	...
	{
		$finder = new Finder( $this->toolPriyx );
		$oodb   = $this->toolPriyx->getRedBean();
...
class Tree {
	 ...
	 * the Tree.
	 *
	 * @param ToolPriyx $toolPriyx toolPriyx
	 */
	public function __construct( ToolPriyx $toolPriyx )
	{
		$this->toolPriyx = $toolPriyx;
		$this->writer  = $toolPriyx->getWriter();
		$this->oodb    = $toolPriyx->getRedBean();
	}
