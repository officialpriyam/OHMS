...
class Whois2 {
    ...
    public function info()
    {
        ...
        if($whois_server == "") {
            throw new Priyx_Exception('No whois server for this tld in list!');
        }
        ...
        if (!$fp) {
            throw new Priyx_Exception('Whois server '. $whois_server .' is not available');
        }
        ...
        if($tldname=="com" || $tldname=="net" || $tldname=="edu") {
            ...
            $fp = fsockopen($whois_server,43);
            if(!is_resource($fp)) {
                throw new \Priyx_Exception('Could not connect to whois :server server', array(':server'=>$whois_server));
            }
