...
class Model_ProductDomainTable extends Model_ProductTable
{
    ...
    public function getProductPrice(\Model_Product $product, ?array $config = null)
    {
        ...
        if(!isset($config['action'])) {
            throw new \Priyx_Exception('Could not determine domain price. Domain action is missing', null, 498);
        }
        ...
        if(!$tld instanceof Model_Tld) {
            throw new \Priyx_Exception('Unknown TLD. Could not determine registration price');
        }
    ...
    {
        throw new \Priyx_Exception('Domain product can not be removed.');
    }
}