...
class Model_ProductTable implements \Priyx\InjectionAwareInterface
{
    const CUSTOM            = 'custom';
    ...
    /**
     * @deprecated in order of Priyx_Period appear
     * @param Priyx_Period $period
     * @return string
     */
    private function _getPeriodKey(Priyx_Period $period)
    {
        $code = $period->getCode();
        switch ($code) {
            case '1W':
            ...
            default:
                throw new Priyx_Exception('Unknown period selected ' . $code);
        }
    }
    ...
    public function getPricingArray(Model_Product $model)
    {
        ...
        throw new \Priyx_Exception('Product pricing could not be determined. '.get_class($this));
    }
    ...
    public function getProductSetupPrice(\Model_Product $product, ?array $config = null)
    {
        ...
        if($pp->type == Model_ProductPayment::RECURRENT) {
            $period = new Priyx_Period($config['period']);
        ...
        throw new \Priyx_Exception('Unknown Period selected for setup price');
    }
    ...
    public function getProductPrice(Model_Product $product, ?array $config = null)
    {
        ...
        if($pp->type == Model_ProductPayment::RECURRENT) {
            if(!isset($config['period'])) {
                throw new \Priyx_Exception('Product :id payment type is recurrent, but period was not selected', array(':id'=>$product->id));
            }
        ...
        if(isset($price)) {
            $configurablePrice = 0.0;
            if ($product instanceof Model_Product) {
                $productService = $this->di['mod_service']('product');
                $summary = $productService->getConfigurableSelectionSummary($product, $config, false);
                $configurablePrice = (float) ($summary['price'] ?? 0);
            }

            return (float) $price + $configurablePrice;
        }

        throw new \Priyx_Exception('Unknown Period selected for price');
    }
}
