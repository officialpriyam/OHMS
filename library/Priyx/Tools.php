...
class Priyx_Tools
{
    ...
    public function url($link = null)
    {
        ...
        return PS_URL .'index.php?_url=/' . $link;
    }
    
    public function hasService($type)
    {
        $file = PS_PATH_MODS . '/mod_'.$type.'/Service.php';
        return file_exists($file);
    }
    
    public function getService($type)
    {
        $class = 'Priyx_Mod_'.ucfirst($type).'_Service';
        $file = PS_PATH_MODS . '/mod_'.$type.'/Service.php';
        if(!file_exists($file)){
            throw new \Priyx_Exception('Service class :class was not found in :path', array(':class'=>$class,':path'=>$file));
        }
        require_once $file;
    	return new $class();
    }
    ...
    {
        $config = json_decode((string)$json_str, true);
    ...
    public function cache_function($buildCallback, array $args = array(), $timeoutSeconds = 3600)
    {
            ...
            $file_path = PS_PATH_CACHE .DIRECTORY_SEPARATOR. md5($cacheKey);
    ...
    public function getTable($type)
    {
        ...
        $file = PS_PATH_LIBRARY . '/Model/'.$type.'Table.php';
        if(!file_exists($file)){
            throw new \Priyx_Exception('Service class :class was not found in :path', array(':class'=>$class,':path'=>$file));
        }
        require_once $file;
