...
class Priyx_Mail
{
    ...
    public function send($transport = 'sendmail', $options = array())
    {
        if($transport == 'sendmail') {
            ...
            throw new \Priyx_Exception('Unknown mail transport: :transport', array(':transport'=>$transport));
        }
    }
    ...
    protected function _sendSendgrid($options)
    {
        if(!isset($options['sendgrid_username']) || !isset($options['sendgrid_password'])) {
            throw new \Priyx_Exception('Sendgrid is not configured');
        }
    ...
    protected function _sendSmtpMail($options)
    {
        if(!isset($options['smtp_host'])) {
            throw new \Priyx_Exception('SMTP host not configured');
        }
        
        if(!isset($options['smtp_port'])) {
            throw new \Priyx_Exception('SMTP port not configured');
        }
        ...
        if(empty($host)) {
            throw new \Priyx_Exception('SMTP hostname is not configured.');
        }
    ...
    protected function _sendMail()
    {
        try {
            ...
            $this->addHeader('X-mailer', 'OHMS/'.Priyx_Version::VERSION);
    ...
    {
        throw new \Priyx_Exception($errstr);
    }
