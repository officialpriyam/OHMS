...
class Priyx_Ftp
{
	...
	public function __construct($opt=array())
    {
		...
		if ( ! extension_loaded('ftp')) {
            throw new \Priyx_Exception('FTP extension is not enabled in PHP. You will not be able to use automatic updates and other automation.');
		}
		...
		//Check if possible to use ftp functions.
		if (!extension_loaded('ftp')) {
			throw new \Priyx_Exception('The ftp PHP extension is not available');
		}
		...
		if ( empty($opt['hostname']) ) {
            throw new \Priyx_Exception('FTP hostname is required');
		} else {
		...
		if ( empty($opt['username']) ) {
            throw new \Priyx_Exception('FTP username is required');
		} else {
			$this->options['username'] = $opt['username'];
        }

		if ( empty($opt['password']) ) {
            throw new \Priyx_Exception('FTP password is required');
		} else {
	...
	public function connect()
    {
		...
		if ( ! $this->link ) {
			throw new \Priyx_Exception('Failed to connect to FTP Server :host : :port',array(':host'=>$this->options['hostname'], ':port'=>$this->options['port']));
		}

		if ( ! @ftp_login($this->link,$this->options['username'], $this->options['password']) ) {
            throw new \Priyx_Exception('FTP Username/Password incorrect for :usernmae', array(':username'=>$this->options['username']));
		}
