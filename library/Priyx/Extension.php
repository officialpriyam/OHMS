 ...
 */
    ...
    /**
     * OHMS extensions API wrapper
     */
    class Priyx_Extension
    {
        ...
        private $_url = 'https://extensions.OHMS.org/api/';

        public function getExtension($id, $type = Priyx_Extension::TYPE_MOD)
        {
            $params = array();
        ...
        public function getLatestExtensionVersion($id, $type = Priyx_Extension::TYPE_MOD)
        {
            $params = array();
            $params['type'] = $type;
            $params['id'] = $id;
        ...
        {
            $params['bb_version'] = Priyx_Version::VERSION;
            $url = $this->_url.$call.'?'.http_build_query($params);
            $curl = new Priyx_Curl($url, 5);
            $curl->request();
            $response = $curl->getBody();
            $json = json_decode($response, 1);

            if(is_null($json)) {
                throw new \Priyx_Exception('Unable to connect to OHMS extensions site.', null, 1545);
            }
