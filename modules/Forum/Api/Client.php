...
class Client extends \Api_Abstract
{
    ...
    public function get($data)
    {
        if (!isset($data['id']) && !isset($data['slug'])) {
            throw new \Priyx_Exception('ID or slug is missing');
        }
        ...
        if (!$model instanceof \Model_Forum) {
            throw new \Priyx_Exception('Forum not found');
        }
    ...
    public function get_topic($data)
    {
        ...
        if (!$model instanceof \Model_ForumTopic) {
            throw new \Priyx_Exception('Forum Topic not found');
        }
        $table->hitView($model);
    ...
    public function start_topic($data)
    {
        if (!isset($data['forum_id'])) {
            throw new \Priyx_Exception('Forum ID not passed');
        }
        if (!isset($data['topic'])) {
            throw new \Priyx_Exception('Forum topic not passed');
        }
        if (!isset($data['message'])) {
            throw new \Priyx_Exception('Forum message not passed');
        }

        if (strlen($data['message']) < 2) {
            throw new \Priyx_Exception('Your message is too short');
        }
        ...
        if (\Model_Forum::STATUS_LOCKED == $forum->status) {
            throw new \Priyx_Exception('Forum is locked. No new topics can be started');
        }
    ...
    public function get_topic_message_list($data)
    {
        if (!isset($data['forum_topic_id'])) {
            throw new \Priyx_Exception('Forum Topic ID not passed');
        }
    ...
    public function post_message($data)
    {
        ...
        if (!isset($data['message'])) {
            throw new \Priyx_Exception('Topic message not passed');
        }
        ...
        if (\Model_ForumTopic::STATUS_LOCKED == $topic->status) {
            throw new \Priyx_Exception('Forum topic is locked. No new message can be posted');
        }
