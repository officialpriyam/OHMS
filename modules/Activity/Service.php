...
class Service implements InjectionAwareInterface
{
    ...
    /** EVENTS  **/
    public static function onAfterClientLogin(\Priyx_Event $event)
    {
    ...
    public static function onAfterAdminLogin(\Priyx_Event $event)
    {
        $params = $event->getParameters();
        $di = $event->getDi();
    ...
    public function getSearchQuery($data)
    {
        ...
        if ($no_info) {
            $where[] = 'm.priority < :priority';
            $params[':priority'] = \Priyx_Log::INFO;
        }

        if ($no_debug) {
            $where[] = 'm.priority < :priority';
            $params[':priority'] = \Priyx_Log::DEBUG;
        }
