...
class Service implements InjectionAwareInterface
{
    ...
    public static function onAfterAdminOrderActivate(\Priyx_Event $event)
    {
    ...
    public static function onAfterAdminOrderRenew(\Priyx_Event $event)
    {
        $params = $event->getParameters();
        $order_id = $params['id'];
        $di = $event->getDi();
    ...
    public static function onAfterAdminOrderSuspend(\Priyx_Event $event)
    {
        $params = $event->getParameters();
        $order_id = $params['id'];
        $di = $event->getDi();
    ...
    public static function onAfterAdminOrderUnsuspend(\Priyx_Event $event)
    {
        $params = $event->getParameters();
        $order_id = $params['id'];
        $di = $event->getDi();
    ...
    public static function onAfterAdminOrderCancel(\Priyx_Event $event)
    {
        $params = $event->getParameters();
        $order_id = $params['id'];
        $di = $event->getDi();
    ...
    public static function onAfterAdminOrderUncancel(\Priyx_Event $event)
    {
        $params = $event->getParameters();
        $order_id = $params['id'];
        $di = $event->getDi();
    ...
    {
        $log = $this->di['logger'];
        $log->addWriter(new \Priyx_LogDb('Model_ClientOrderStatus'));
    ...
    public function createOrder(\Model_Client $client, \Model_Product $product, array $data)
    {
        ...
        if (!$currency instanceof \Model_Currency) {
            throw new \Priyx_Exception('Currency could not be determined for order');
        }
        ...
        if (!$cartService->isStockAvailable($product, $qty)) {
            throw new \Priyx_Exception('Product :id is out of stock.', [':id' => $product->id], 831);
        }

        // Addons must have defined master order
        $parent_order = false;
        if ($product->is_addon && empty($group_id)) {
            throw new \Priyx_Exception('Group ID parameter is missing for addon product order', null, 832);
        }

        if (!empty($group_id)) {
            $parent_order = $this->getMasterOrderForClient($client, $group_id);
            if (!$parent_order instanceof \Model_ClientOrder) {
                throw new \Priyx_Exception('Parent order :group_id was not found', [':group_id' => $group_id]);
            }
        }
        ...
        $productModule = $this->di['mod_service']('product');
        $productModule->validateConfigurableSelections($product, $config);
        $configurableSummary = $productModule->getConfigurableSelectionSummary($product, $config, false);
        if (!empty($configurableSummary['items'])) {
            $config['config_options_data'] = $configurableSummary['items'];
            $config['config_options_price'] = $configurableSummary['price'];
        }
     ...
     *
     * @see https://github.com/OHMS/OHMS/issues/54
     */
    ...
    public function activateOrder(\Model_ClientOrder $order, $data = [])
    {
        ...
        if (!in_array($order->status, $statues)) {
            if (!isset($data['force']) || !$data['force']) {
                throw new \Priyx_Exception('Only pending setup or failed orders can be activated');
            }
        }
    ...
    public function createFromOrder(\Model_ClientOrder $order)
    {
        ...
        if (!is_object($service)) {
            ...
            if (method_exists($s, 'create') || method_exists($s, 'action_create')) {
                ...
                if (!is_object($service)) {
                    throw new \Priyx_Exception('Error creating '.$order->service_type.' service for order '.$order->id);
                }
    ...
    protected function _callOnService(\Model_ClientOrder $order, $action)
    {
        ...
        if (in_array($order->service_type, $core_services)) {
            ...
            if (!method_exists($repo, $m) || !is_callable([$repo, $m])) {
                throw new \Priyx_Exception('Service '.$order->service_type.' do not support '.$m);
            }
    ...
    public function suspendFromOrder(\Model_ClientOrder $order, $reason = null, $skipEvent = false)
    {
        ...
        if (\Model_ClientOrder::STATUS_ACTIVE != $order->status) {
            throw new \Priyx_Exception('Only active orders can be suspended');
        }
    ...
    public function cancelFromOrder(\Model_ClientOrder $order, $reason = null, $skipEvent = false)
    {
        ...
        if (in_array($order->status, [\Model_ClientOrder::STATUS_CANCELED, \Model_ClientOrder::STATUS_PENDING_SETUP, \Model_ClientOrder::STATUS_FAILED_SETUP])) {
            throw new \Priyx_Exception('Can not cancel '.$order->status.' order');
        }
