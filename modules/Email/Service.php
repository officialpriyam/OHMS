...
class Service implements \Priyx\InjectionAwareInterface
{
    ...
    public function sendTemplate($data)
    {
        ...
        if (!isset($data['to']) && !isset($data['to_staff']) && !isset($data['to_client'])) {
            throw new \Priyx_Exception('Receiver is not defined. Define to or to_client or to_staff parameter');
        }
        $vars = $data;
        ...
        if (!$from) {
            throw new \Priyx_Exception('From email address can not be empty');
        }
    ...
    private function _getDefaults($data)
    {
        ...
        $mod = $matches[1];
        $path = PS_PATH_MODS.'/'.ucfirst($mod).'/html_email/'.$code.'.phtml';
    ...
    private function _getVarsString()
    {
        ...
        $str .= 'This email template was automatically generated by OHMS extension.   '.PHP_EOL;
        $str .= 'Template is ready to be modified.   '.PHP_EOL;
        $str .= 'Email template is just like OHMS theme file.   '.PHP_EOL;
    ...
    public function resetTemplateByCode($code)
    {
        ...
        if (!$t instanceof \Model_EmailTemplate) {
            throw new \Priyx_Exception('Email template :code was not found', [':code' => $code]);
        }
    ...
    public function getEmailById($id)
    {
        ...
        if (!$model instanceof \Model_ActivityClientEmail) {
            throw new \Priyx_Exception('Email not found');
        }
    ...
    {
        $pattern = PS_PATH_MODS.'/*/html_email/*.phtml';
        $list = glob($pattern);
