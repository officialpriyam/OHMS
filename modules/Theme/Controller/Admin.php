...
namespace Priyx\Mod\Theme\Controller;
...
class Admin implements \Priyx\InjectionAwareInterface
{
     ...
     * @return array
     */
    public function save_theme_settings(\Priyx_App $app, $theme)
    {
        ...
        try {
            if (!$t->isAssetsPathWritable()) {
                throw new \Priyx_Exception('Theme ":name" assets folder is not writable. Files can not be uploaded and settings can not be saved. Set folder permissions to 777', [':name' => $t->getName()]);
            }
    ...
    public function get_theme(\Priyx_App $app, $theme)
    {
        ...
        $data = [
            ...
            'orderform_pairs' => $service->getOrderformPairs($t),
        ];
