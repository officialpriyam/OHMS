...
namespace Priyx\Mod\Theme\Model;
...
class Theme
{
    ...
    public function __construct($name)
    {
        ...
        if (!file_exists($this->getPath())) {
            throw new \Priyx_Exception('Theme ":name" does not exists', [':name' => $name]);
        }
    }
    ...
    public function getUploadedAssets()
    {
        ...
        foreach ($files as $file) {
            $assetFile = str_replace('_', '.', $file);
            if (file_exists($assets_folder.DIRECTORY_SEPARATOR.$assetFile)) {
                $uploaded[] = [
                    'name' => $assetFile,
                    'url' => $this->getUrl().'/assets/'.$assetFile,
                ];
            }
        }
    ...
    public function getSettingsPageFiles()
    {
    ...
    {
        $prefix = $this->isAdminAreaTheme() ? 'admin/template/' : 'themes/';

        return PS_URL.$prefix.$this->name;
    }

    public function getPath()
    {
        $path = $this->isAdminAreaTheme() ? PS_PATH_ADMIN_THEMES : PS_PATH_THEMES;
    ...
    public function getPathOrderforms()
    {
        return $this->getPath().DIRECTORY_SEPARATOR.'orderform';
    }
