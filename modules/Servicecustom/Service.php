...
namespace Priyx\Mod\Servicecustom;
...
class Service implements \Priyx\InjectionAwareInterface
{
            ...
            foreach ($form['fields'] as $field) {
                if (1 == $field['required']) {
                    ...
                    if (!isset($data[$field_name]) || empty($data[$field_name])) {
                        throw new \Priyx_Exception('You must fill in all required fields. '.$field['label'].' is missing', null, 9684);
                    }
                }
                ...
                if (1 == $field['readonly']) {
                    ...
                    if ($data[$field_name] != $field['default_value']) {
                        throw new \Priyx_Exception('Field '.$field['label'].' is read only. You can not change its value', null, 5468);
                    }
                }
            }
        }
    ...
    public function customCall(\Model_ServiceCustom $model, $method, $params = [])
    {
        ...
        if (in_array($method, $forbidden_methods)) {
            throw new \Priyx_Exception('Custom plugin method :method is forbidden', [':method' => $method], 403);
        }
    ...
    {
        if (!is_array($config)) {
            throw new \Priyx_Exception('Config must be an array');
        }
    ...
    private function callOnAdapter(\Model_ServiceCustom $model, $method, $params = [])
    {
        ...
        if (APPLICATION_ENV != 'testing' && !file_exists(PS_PATH_LIBRARY.DIRECTORY_SEPARATOR.$file)) {
            $e = new \Priyx_Exception('Plugin class file :file was not found', [':file' => $file], 3124);
            if (PS_DEBUG) {
        ...
        if (!method_exists($adapter, $method)) {
            throw new \Priyx_Exception('Plugin :plugin does not support action :action', [':plugin' => $plugin, ':action' => $method], 3125);
        }
