...
namespace Priyx\Mod\Support;

class Service implements \Priyx\InjectionAwareInterface
{
    private const MAX_INLINE_TEXT_LENGTH = 255;
    ...
    public static function onAfterClientOpenTicket(\Priyx_Event $event)
    {
        $di = $event->getDi();
    ...
    public static function onAfterAdminOpenTicket(\Priyx_Event $event)
    {
        $di = $event->getDi();
        $supportService = $di['mod_service']('support');
        $emailService = $di['mod_service']('email');
    ...
    public static function onAfterAdminCloseTicket(\Priyx_Event $event)
    {
        $di = $event->getDi();
        $supportService = $di['mod_service']('support');
        $emailService = $di['mod_service']('email');
    ...
    public static function onAfterAdminReplyTicket(\Priyx_Event $event)
    {
        $di = $event->getDi();
        $supportService = $di['mod_service']('support');
        $emailService = $di['mod_service']('email');
    ...
    public static function onAfterGuestPublicTicketOpen(\Priyx_Event $event)
    {
        $di = $event->getDi();
        $supportService = $di['mod_service']('support');
        $emailService = $di['mod_service']('email');
    ...
    public static function onAfterAdminPublicTicketOpen(\Priyx_Event $event)
    {
        $di = $event->getDi();
        $supportService = $di['mod_service']('support');
        $emailService = $di['mod_service']('email');
    ...
    public static function onAfterAdminPublicTicketReply(\Priyx_Event $event)
    {
        $di = $event->getDi();
        $supportService = $di['mod_service']('support');
        $emailService = $di['mod_service']('email');
    ...
    public static function onAfterAdminPublicTicketClose(\Priyx_Event $event)
    {
        $di = $event->getDi();
        $supportService = $di['mod_service']('support');
        $emailService = $di['mod_service']('email');
    ...
    public function findOneByClient(\Model_Client $c, $id)
    {
        ...
        if (!$ticket instanceof \Model_SupportTicket) {
            throw new \Priyx_Exception('Ticket not found');
        }
    ...
    public function helpdeskRm(\Model_SupportHelpdesk $model)
    {
        ...
        if (count($tickets) > 0) {
            throw new \Priyx_Exception('Can not remove helpdesk which has tickets');
        }
        $this->di['db']->trash($model);
    ...
    {
        $model->content = $this->normalizeTicketMessage($content);
        $model->updated_at = date('Y-m-d H:i:s');
    ...
    {
        $content = $this->normalizeTicketMessage($content);
    ...
    public function ticketCreateForAdmin(\Model_Client $client, \Model_SupportHelpdesk $helpdesk, $data, \Model_Admin $identity)
    {
        ...
        $subject = $this->normalizeInlineText($this->di['array_get']($data, 'subject', ''), 'subject');
        $content = $this->normalizeTicketMessage($this->di['array_get']($data, 'content', ''));
        ...
        $this->messageCreateForTicket($ticket, $identity, $content);
    ...
    public function ticketCreateForGuest($data)
    {
        ...
        $authorName = $this->normalizeInlineText($this->di['array_get']($data, 'name', ''), 'name');
        $subject = $this->normalizeInlineText((string) $subject, 'subject');
        $message = $this->normalizeTicketMessage((string) $message);

        $ticket = $this->di['db']->dispense('SupportPTicket');
        $ticket->hash = sha1(random_bytes(13));
        $ticket->author_name = $authorName;
    ...
    public function canClientSubmitNewTicket(\Model_Client $client, array $config)
    {
        ...
        if ($timeSinceLast < $hours) {
            throw new \Priyx_Exception(sprintf('You can submit one ticket per %s hours. %s hours left', $hours, $hours - $timeSinceLast));
        }
    ...
    public function ticketCreateForClient(\Model_Client $client, \Model_SupportHelpdesk $helpdesk, array $data)
    {
        ...
        if (!is_null($rel_id) && \Model_SupportTicket::REL_TYPE_ORDER == $rel_type) {
            ...
            if (!$o instanceof \Model_ClientOrder) {
                throw new \Priyx_Exception('Order ID does not exist');
            }
        }
        ...
        if ($rel_id && $rel_type && $rel_task && $this->checkIfTaskAlreadyExists($client, $rel_id, $rel_type, $rel_task)) {
            throw new \Priyx_Exception('We have already received this request.');
        }
    ...
    public function messageCreateForTicket(\Model_SupportTicket $ticket, $identity, $content)
    {
        ...
        if ($identity instanceof \Model_Admin) {
            ...
            throw new \Priyx_Exception('Identity is not valid');
        }
        $msg->content = $content;
        $msg->ip = $this->di['request']->getClientAddress();
        $msg->created_at = date('Y-m-d H:i:s');
    ...
    public function publicFindOneByHash($hash)
    {
        ...
        if (!$publicTicket instanceof \Model_SupportPTicket) {
            throw new \Priyx_Exception('Public ticket not found');
        }
    ...
    {
        $message = $this->normalizeTicketMessage($message);
        $msg = $this->di['db']->dispense('SupportPTicketMessage');
        $msg->support_p_ticket_id = $ticket->id;
        $msg->content = $message;
        $msg->ip = $this->di['request']->getClientAddress();
    ...
    public function cannedCreate($title, $categoryId, $content = null)
    {
        ...
        $model->title = $this->normalizeInlineText($title, 'title');
        $model->content = $this->normalizeOptionalTicketMessage($content);
        $model->created_at = date('Y-m-d H:i:s');
        $model->updated_at = date('Y-m-d H:i:s');
        $id = $this->di['db']->store($model);
    ...
    public function cannedUpdate(\Model_SupportPr $model, $data)
    {
        ...
        $model->title = $this->normalizeInlineText($this->di['array_get']($data, 'title', $model->title), 'title');
        $model->content = $this->normalizeOptionalTicketMessage($this->di['array_get']($data, 'content', $model->content));
        $model->updated_at = date('Y-m-d H:i:s');
        $this->di['db']->store($model);
    ...
    public function noteCreate(\Model_SupportTicket $ticket, \Model_Admin $identity, $note)
    {
        ...
        $model->note = $this->normalizeTicketMessage($note);
        $model->created_at = date('Y-m-d H:i:s');
        $model->updated_at = date('Y-m-d H:i:s');
        $id = $this->di['db']->store($model);
    ...
    private function normalizeTicketMessage($content)
    {
        return $this->normalizeMultilineText($content, self::MAX_MESSAGE_LENGTH, 'message');
    }

    private function normalizeOptionalTicketMessage($content)
    {
        if (null === $content) {
            return null;
        }

        $content = $this->normalizeOptionalMultilineText($content, self::MAX_MESSAGE_LENGTH);
        if (null === $content) {
            return null;
        }
    ...
    private function normalizeInlineText($value, $field)
    {
        $value = $this->normalizeMultilineText($value, self::MAX_INLINE_TEXT_LENGTH, $field);
        $value = preg_replace('/\s+/u', ' ', str_replace("\n", ' ', $value));

        return trim($value);
    }

    private function normalizeMultilineText($value, $maxLength, $field)
    {
        ...
        if ('' === $value) {
            throw new \Priyx_Exception(ucfirst($field) . ' is empty');
        }

        if ($this->stringLength($value) > $maxLength) {
            $value = $this->stringSlice($value, $maxLength);
        }

        return $value;
    }

    private function normalizeOptionalMultilineText($value, $maxLength)
    {
        $value = is_string($value) ? $value : (string) $value;
        $value = str_replace(["\r\n", "\r"], "\n", $value);
        $value = preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]+/u', '', $value);
    ...
    private function stringSlice($value, $length)
    {
