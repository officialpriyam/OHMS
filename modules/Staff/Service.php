...
class Service implements InjectionAwareInterface
{
    ...
    public function login($email, $password, $ip)
    {
        ...
        if (!$model instanceof \Model_Admin) {
            ...
            throw new \Priyx_Exception('Check your login details', null, 403);
        }
        ...
        $this->di['session']->regenerateId(true);
    ...
    public static function onAfterClientOrderCreate(\Priyx_Event $event)
    {
        $di = $event->getDi();
    ...
    public static function onAfterClientReplyTicket(\Priyx_Event $event)
    {
        $params = $event->getParameters();
        $di = $event->getDi();
    ...
    public static function onAfterClientCloseTicket(\Priyx_Event $event)
    {
        $params = $event->getParameters();
        $di = $event->getDi();
    ...
    public static function onAfterClientSignUp(\Priyx_Event $event)
    {
        $params = $event->getParameters();
        $di = $event->getDi();
    ...
    public static function onAfterGuestPublicTicketReply(\Priyx_Event $event)
    {
        $params = $event->getParameters();
        $di = $event->getDi();
    ...
    public static function onAfterGuestPublicTicketClose(\Priyx_Event $event)
    {
        $params = $event->getParameters();
        $di = $event->getDi();
    ...
    {
        if ($model->protected) {
            throw new \Priyx_Exception('This administrator account is protected and can not be removed');
        }
    ...
    public function create(array $data)
    {
        ...
        try {
            ...
            throw new \Priyx_Exception('Staff member with email :email is already registered', [':email' => $data['email']], 788954);
        }
    ...
    public function deleteGroup(\Model_AdminGroup $model)
    {
        $id = $model->id;
        if (1 == $model->id) {
            throw new \Priyx_Exception('Administrators group can not be removed');
        }
        ...
        if ($staffMembersInGroup > 0) {
            throw new \Priyx_Exception('Can not remove group which has staff members');
        }
    ...
    protected function _sendMail($admin, $admin_pass)
    {
        ...
        $content .= 'Read OHMS documentation to get started http://docs.OHMS.org/'.PHP_EOL;
