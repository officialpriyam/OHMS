...
class Guest extends \Api_Abstract
{
     ...
     * Used by automated OHMS installer.
     *
    ...
    public function create($data)
    {
        ...
        if (!$allow) {
            throw new \Priyx_Exception('Administrator account already exists', null, 55);
        }
        $required = [
        ...
        if (isset($config['allowed_ips']) && isset($config['check_ip']) && $config['check_ip']) {
            ...
            if (!empty($allowed_ips)) {
                ...
                if (!in_array($this->getIp(), $allowed_ips)) {
                    throw new \Priyx_Exception('You are not allowed to login to admin area from :ip address', [':ip' => $this->getIp()], 403);
                }
            }
        }
