...
class Guest extends \Api_Abstract
{
    ...
    public function get($data)
    {
        ...
        if (!$model) {
            throw new \Priyx_Exception('Invoice was not found');
        }
    ...
    public function update($data)
    {
        ...
        if ('paid' == $invoice->status) {
            throw new \Priyx_Exception('Paid Invoice can not be modified');
        }
    ...
    {
        if (!isset($data['hash'])) {
            throw new \Priyx_Exception('Invoice hash not passed. Missing param hash', null, 810);
        }

        if (!isset($data['gateway_id'])) {
            throw new \Priyx_Exception('Payment method not found. Missing param gateway_id', null, 811);
        }
