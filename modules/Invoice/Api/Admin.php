...
namespace Priyx\Mod\Invoice\Api;
...
class Admin extends \Api_Abstract
{
    ...
    public function renewal_invoice($data)
    {
        ...
        if ($model->price <= 0) {
            throw new \Priyx_Exception('Order :id is free. No need to generate invoice.', [':id' => $model->id]);
        }
     ...
     * @optional int $gateway_id - Payment gateway ID on OHMS
     * @optional float $amount - Transaction amount
     * @optional string $currency - Currency code. Must be available on OHMS
     * @optional string $type - Currency code. Must be available on OHMS
     * @optional string $status - Transaction status on OHMS
     ...
     *
     * @param int $bb_invoice_id - OHMS invoice id
     * @param int $bb_gateway_id - OHMS gateway id
     *
    ...
    public function subscription_create($data)
    {
        ...
        if ($client->currency != $data['currency']) {
            throw new \Priyx_Exception('Client currency must match subscription currency. Check if clients currency is defined.');
        }
        $subscriptionService = $this->di['mod_service']('Invoice', 'Subscription');
    ...
    public function subscription_get($data)
    {
        ...
        if (!$model instanceof \Model_Subscription) {
            throw new \Priyx_Exception('Subscription not found');
        }
