...
class ServiceTransaction implements InjectionAwareInterface
{
    ...
    public function create(array $data)
    {
        ...
        if (!$skip_validation) {
            if (!isset($data['bb_invoice_id'])) {
                throw new \Priyx_Exception('Transaction invoice id is missing');
            }

            if (!isset($data['bb_gateway_id'])) {
                throw new \Priyx_Exception('Payment gateway id is missing');
            }
    ...
    public function processTransaction($id)
    {
        ...
        if (!$tx) {
            throw new \Priyx_Exception('Transaction :id not found.', ['id' => $id], 404);
        }

        if (empty($tx->gateway_id)) {
            throw new \Priyx_Exception('Could not determine transaction origin. Transaction payment gateway is unknown.', null, 701);
        }

        $gtw = $this->di['db']->load('PayGateway', $tx->gateway_id);
        if (!$gtw instanceof \Model_PayGateway) {
            throw new \Priyx_Exception('Can not handle transaction received from unknown payment gateway: :id', [':id' => $tx->gateway_id], 704);
        }

        $payGatewayService = $this->di['mod_service']('Invoice', 'PayGateway');
        $adapter = $payGatewayService->getPaymentAdapter($gtw);
        if (!method_exists($adapter, 'processTransaction')) {
            throw new \Priyx_Exception('Payment adapter :adapter does not support action :action', [':adapter' => $gtw->name, ':action' => 'processTransaction'], 705);
        }
    ...
    public function process($tx)
    {
        ...
        try {
            ...
            switch ($transaction->type) {
                ...
                default:
                    throw new \Priyx_Exception('Unknown transaction #:id type: :type', [':id' => $transaction->id, ':type' => $transaction->type], 632);
            }
        } catch (\Exception $e) {
    ...
    private function _parseIpnAndApprove(\Model_Transaction &$tx)
    {
        ...
        if (!$tx->invoice_id) {
            throw new \Priyx_Exception('Transaction :id is not associated with an invoice.', [':id' => $tx->id], 702);
        }

        $invoice = $this->di['db']->load('Invoice', $tx->invoice_id);
        if (!$invoice instanceof \Model_Invoice) {
            throw new \Priyx_Exception('Invoice #:id not found', [':id' => $tx->invoice_id], 703);
        }
        ...
        if (APPLICATION_ENV != 'testing' && $tx->validate_ipn) {
            if (!$adapter->isIpnValid($ipn, $mpi)) {
                ...
                throw new \Priyx_Exception('Instant payment notification (IPN) did not pass gateway :id validation', [':id' => $gtw->gateway], 706);
            }
            $tx->output = $adapter->getOutput();
        }

        if (!method_exists($adapter, 'getTransaction')) {
            throw new \Priyx_Exception('Payment adapter :adapter does not support action :action', [':adapter' => $gtw->name, ':action' => 'getTransaction'], 705);
        }

        $response = $adapter->getTransaction($ipn, $mpi);
        if (!$response instanceof \Payment_Transaction) {
            throw new \Priyx_Exception('Payment gateway :id method getTransaction should return Payment_Transaction object', [':id' => $gtw->gateway], 705);
        }
    ...
    private function _subscribe(\Model_Transaction $tx)
    {
        ...
        if (empty($tx->s_id)) {
            throw new \Priyx_Exception('Can not create subscription. Subscription id from payment gateway was not received');
        }
    ...
    private function _unsubscribe(\Model_Transaction $tx)
    {
        ...
        if (!$model instanceof \Model_Subscription) {
            throw new \Priyx_Exception('Subscription #:id was not found. Could not unsubscribe', [':id' => $tx->s_id]);
        }
    ...
    {
        if (\Model_Transaction::STATUS_APPROVED != $tx->status) {
            throw new \Priyx_Exception('Only approved transaction can be processed');
        }

        if (empty($tx->invoice_id)) {
            throw new \Priyx_Exception('Transaction :id is not associated with an invoice.', [':id' => $tx->id], 7022);
        }
        ...
        if ($invoice->currency != $tx->currency) {
            throw new \Priyx_Exception('Transaction currency :code do not match required currency :required', [':code' => $tx->currency, ':required' => $invoice->currency], 709);
        }

        // check that payment status is completed if
        if (\Payment_Transaction::STATUS_PENDING == $tx->txn_status) {
            throw new \Priyx_Exception('Transaction status on payment gateway is Pending. Only Complete or Unknown transactions can be processed.', null, 712);
        }
    }
    ...
    public function debitTransaction(\Model_Transaction $tx)
    {
        ...
        if ($client->currency != $proforma->currency) {
            throw new \Priyx_Exception('Client currency do not match invoice currency');
        }

        // do not debit negative or zero amount
        if ($tx->amount < 0) {
            throw new \Priyx_Exception('Can not add negative amount to client balance for debit transaction');
        }
