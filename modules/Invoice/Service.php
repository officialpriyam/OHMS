...
class Service implements InjectionAwareInterface
{
    ...
    public static function onAfterAdminInvoicePaymentReceived(\Priyx_Event $event)
    {
    ...
    public static function onAfterAdminInvoiceApprove(\Priyx_Event $event)
    {
        $params = $event->getParameters();
        $di = $event->getDi();
        $service = $di['mod_service']('invoice');
    ...
    public static function onAfterAdminInvoiceReminderSent(\Priyx_Event $event)
    {
        $params = $event->getParameters();
        $di = $event->getDi();
        $service = $di['mod_service']('invoice');
    ...
    public static function onAfterAdminCronRun(\Priyx_Event $event)
    {
        $di = $event->getDi();
    ...
    public static function onEventAfterInvoiceIsDue(\Priyx_Event $event)
    {
        $params = $event->getParameters();
        $di = $event->getDi();
        $service = $di['mod_service']('invoice');
    ...
    public function refundInvoice(\Model_Invoice $invoice, $note = null)
    {
        ...
        switch ($logic) {
            ...
            case 'negative_invoice':
                ...
                if ($total <= 0) {
                    throw new \Priyx_Exception('Can not refund invoice with negative amount');
                }
    ...
    public function deleteInvoiceByClient(\Model_Invoice $model)
    {
        ...
        foreach ($invoiceItem as $item) {
            if (\Model_InvoiceItem::TYPE_ORDER == $item->type) {
                throw new \Priyx_Exception('Invoice is related to order #:id. Please cancel order first.', [':id' => $item->rel_id]);
            }
        }
    ...
    public function generateForOrder(\Model_ClientOrder $order, $due_days = null)
    {
        ...
        if ($order->price <= 0) {
            throw new \Priyx_Exception('Invoices are not generated for 0 amount orders');
        }
    ...
    {
        if (!$client->currency) {
            throw new \Priyx_Exception('You must have at least one active order before you can add funds so you cannot proceed at the current time!');
        }
        ...
        if ($min_amount && $amount < $min_amount) {
            throw new \Priyx_Exception('Amount is not valid', null, 981);
        }

        if ($max_amount && $amount > $max_amount) {
            throw new \Priyx_Exception('Amount is not valid', null, 982);
        }
    ...
    public function processInvoice(array $data)
    {
        ...
        if (!$invoice instanceof \Model_Invoice) {
            throw new \Priyx_Exception('Invoice not found', null, 812);
        }

        $gtw = $this->di['db']->load('PayGateway', $data['gateway_id']);
        if (!$gtw instanceof \Model_PayGateway) {
            throw new \Priyx_Exception('Payment method not found', null, 813);
        }

        if (!$gtw->enabled) {
            throw new \Priyx_Exception('Payment method not enabled', null, 814);
        }
        ...
        // @bug https://github.com/OHMS/OHMS/issues/108
    ...
    public function generatePDF($hash, $identity)
    {
        ...
        if (!$invoice instanceof \Model_Invoice) {
            throw new \Priyx_Exception('Invoice not found');
        }
        ...
        if (isset($company['logo_url']) && !empty($company['logo_url'])) {
            ...
            if ('.png' === substr($url, -4)) {
                ...
                if ($img) {
                    ...
                    throw new \Priyx_Exception('Error converting image to .png');
                }
            }
        }
    ...
    {
        /**
         * @var \Priyx\Mod\System\Service $systemService
         */
        $systemService = $this->di['mod_service']('system');
