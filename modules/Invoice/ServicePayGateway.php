...
class ServicePayGateway implements InjectionAwareInterface
{
    ...
    public function getAvailable()
    {
        ...
        $pattern = PS_PATH_LIBRARY.'/Payment/Adapter/*.php';
        $adapters = [];
    ...
    public function install($code)
    {
        ...
        if (!in_array($code, $available)) {
            throw new \Priyx_Exception('Payment gateway is not available for installation.');
        }
    ...
    public function toApiArray(\Model_PayGateway $model, $deep = false, $identity = null)
    {
        ...
        if ($identity instanceof \Model_Admin) {
            ...
            $result['config'] = is_string($model->config) && $model->config !== ''
                ? (json_decode($model->config, true) ?: [])
                : [];
    ...
    public function getPaymentAdapter(\Model_PayGateway $pg, ?\Model_Invoice $model = null, $optional = [])
    {
        ...
        if (!class_exists($class)) {
            throw new \Priyx_Exception('Payment gateway :adapter was not found', [':adapter' => $class]);
        }
    ...
    {
        $class = $this->getAdapterClassName($pg);
        if (!file_exists(PS_PATH_LIBRARY.'/Payment/Adapter/'.$pg->gateway.'.php')) {
            throw new \Priyx_Exception('Payment gateway :adapter was not found', [':adapter' => $pg->gateway]);
        }

        if (!class_exists($class)) {
            throw new \Priyx_Exception("Payment gateway class $class was not found");
        }
     ...
     */
    public function getCallbackUrl(\Model_PayGateway $pg, ?\Model_Invoice $model = null)
    {
        $p = [
            'bb_gateway_id' => $pg->id,
        ];
     ...
     * @param \Model_Invoice $model
     */
    private function getReturnUrl(\Model_PayGateway $pg, ?\Model_Invoice $model = null)
    {
        if ($model instanceof \Model_Invoice) {
     ...
     * @param \Model_Invoice $model
     */
    private function getCancelUrl(\Model_PayGateway $pg, ?\Model_Invoice $model = null)
    {
        if ($model instanceof \Model_Invoice) {
     ...
     * @param \Model_Invoice $model
     */
    private function getCallbackRedirect(\Model_PayGateway $pg, ?\Model_Invoice $model = null)
    {
        $p = [
            'bb_gateway_id' => $pg->id,
        ];
