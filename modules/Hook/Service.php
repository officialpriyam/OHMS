...
class Service implements InjectionAwareInterface
{
    ...
    public static function onAfterAdminActivateExtension(\Priyx_Event $event)
    {
    ...
    public static function onAfterAdminDeactivateExtension(\Priyx_Event $event)
    {
        $di = $event->getDi();
        $params = $event->getParameters();
        if ('mod' == $params['type']) {
            ...
            if ($mod->hasService()) {
                ...
                foreach ($reflector->getMethods() as $method) {
                    ...
                    if ($method->isPublic()
                        && isset($p[0])
                        && $p[0]->getclass()
                        && in_array($p[0]->getclass()->getName(), ['Priyx_Event', '\Priyx_Event'])) {
