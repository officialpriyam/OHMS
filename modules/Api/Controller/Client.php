...
namespace Priyx\Mod\Api\Controller;
...
class Client implements InjectionAwareInterface
{
    ...
    public function show_error(\Priyx_App $app, $page)
    {
        $exc = new \Priyx_Exception('Unknown API call :call', [':call' => $page], 879);

        return $this->renderJson(null, $exc);
    }

    public function get_method(\Priyx_App $app, $role, $class, $method)
    {
        $call = $class.'_'.$method;
    ...
    public function post_method(\Priyx_App $app, $role, $class, $method)
    {
        $p = $_POST;
    ...
    {
        // snake oil: check request is from the same domain as OHMS is installed if present
    ...
    private function checkAllowedIps()
    {
        ...
        if (!empty($ips) && !in_array($this->_getIp(), $ips)) {
            throw new \Priyx_Exception('Unauthorized IP', null, 1002);
        }
    ...
    private function getAuth()
    {
        ...
        if (!isset($_SERVER['PHP_AUTH_USER'])) {
            throw new \Priyx_Exception('Authentication Failed', null, 201);
        }

        if (!isset($_SERVER['PHP_AUTH_PW'])) {
            throw new \Priyx_Exception('Authentication Failed', null, 202);
        }

        if (empty($_SERVER['PHP_AUTH_PW'])) {
            throw new \Priyx_Exception('Authentication Failed', null, 206);
        }
        ...
        switch ($username) {
            case 'client':
                ...
                if (!$model instanceof \Model_Client) {
                    throw new \Priyx_Exception('Authentication Failed', null, 204);
                }
            ...
            case 'admin':
                ...
                if (!$model instanceof \Model_Admin) {
                    throw new \Priyx_Exception('Authentication Failed', null, 205);
                }
                $sessionAdminArray = [
                    'id' => $model->id,
                    'email' => $model->email,
            ...
            default:
                throw new \Priyx_Exception('Authentication Failed', null, 203);
        }
    }
    ...
    private function isRoleAllowed($role)
    {
        ...
        if (!in_array($role, $allowed)) {
            throw new \Priyx_Exception('Unknow API call', null, 701);
        }
    ...
    public function renderJson($data = null, ?\Exception $e = null)
    {
        ...
        header('X-OHMS-Version: '.\Priyx_Version::VERSION);
