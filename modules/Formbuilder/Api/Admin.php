...
class Admin extends \Api_Abstract
{
    ...
    public function create_form($data)
    {
        ...
        if (isset($data['type']) && ('horizontal' != strtolower($data['type']) || 'default' != strtolower($data['type']))) {
            throw new \Priyx_Exception('Form style was not found in predefined list', null, 3657);
        }
    ...
    public function add_field($data)
    {
        ...
        if (!isset($data['type']) || !$service->typeValidation($data['type'])) {
            throw new \Priyx_Exception('Form field type is not valid', null, 2684);
        }
        if (isset($data['options']) && is_array(($data['options'])) && !$service->isArrayUnique($data['options'])) {
            throw new \Priyx_Exception('This input type must have unique values', null, 3658);
        }
        if (!isset($data['form_id'])) {
            throw new \Priyx_Exception('Form id was not passed', null, 9846);
        }
    ...
    {
        if (!isset($data['form_id'])) {
            throw new \Priyx_Exception('Form id was not passed', null, 9958);
        }
        if (!isset($data['name'])) {
            throw new \Priyx_Exception('Form name was not passed', null, 9842);
        }
    ...
    public function update_form_settings($data)
    {
        if (!isset($data['form_id']) || ('' == trim($data['form_id']))) {
            throw new \Priyx_Exception('Form id was not passed', null, 1654);
        }
        if (!isset($data['form_name'])) {
            throw new \Priyx_Exception('Form name was not passed', null, 9241);
        }

        if (!isset($data['type'])) {
            throw new \Priyx_Exception('Form type was not passed', null, 3794);
        }

        if ('horizontal' != $data['type'] && 'default' != $data['type']) {
            throw new \Priyx_Exception('Field type not supported', null, 3207);
        }
