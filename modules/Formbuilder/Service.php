...
namespace Priyx\Mod\Formbuilder;
...
class Service implements InjectionAwareInterface
{
    ...
    private function slugify($text)
    {
        ...
        if (is_numeric(substr($text, 0, 1))) {
            throw new \Priyx_Exception('Field name can not start with number.', null, 1649);
        }

        if (empty($text)) {
            throw new \Priyx_Exception('Field name can not be empty. Please make sure it is not empty and does not contain special characters.', null, 3502);
        }
    ...
    public function updateField($field)
    {
        ...
        if ($this->formFieldNameExists(['form_id' => $field['form_id'], 'field_name' => $field['name'], 'field_id' => $fieldId])) {
            throw new \Priyx_Exception('Unfortunately field with this name exists in this form already. Form must have different field names.', null, 7628);
        }
        ...
        if (isset($field['type'])) {
            if ('checkbox' == $field['type'] || 'radio' == $field['type'] || 'select' == $field['type']) {
                if (!$this->isArrayUnique(array_filter($field['values'], 'strlen'))) {
                    throw new \Priyx_Exception(ucfirst($field['type']).' values must be unique', null, 1597);
                }
                if (!$this->isArrayUnique(array_filter($field['labels'], 'strlen'))) {
                    throw new \Priyx_Exception(ucfirst($field['type']).' labels must be unique', null, 1598);
                }
            ...
            if ('textarea' == $field['type']) {
                if (count($field['textarea_size']) != count(array_filter($field['textarea_size'], 'is_numeric'))) {
                    throw new \Priyx_Exception('Textarea size options must be integer values', null, 3510);
                }
