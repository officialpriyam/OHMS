...
namespace Priyx\Mod\Servicedomain;
...
class Service implements \Priyx\InjectionAwareInterface
{
    ...
    public function validateOrderData(&$data)
    {
        ...
        if (!in_array($action, ['register', 'transfer', 'owndomain'])) {
            throw new \Priyx_Exception('Invalid domain action.');
        }

        if ('owndomain' == $action) {
            if (!isset($data['owndomain_sld'])) {
                throw new \Priyx_Exception('Order data must contain :field configuration field', [':field' => 'owndomain_sld']);
            }

            if (!$validator->isSldValid($data['owndomain_sld'])) {
                $safe_dom = htmlspecialchars($data['owndomain_sld'], ENT_QUOTES | ENT_HTML5, 'UTF-8');
                throw new \Priyx_Exception('Domain name :domain is not valid', [':domain' => $safe_dom]);
            }
        ...
        if ('transfer' == $action) {
            if (!isset($data['transfer_sld'])) {
                throw new \Priyx_Exception('Order data must contain :field configuration field', [':field' => 'transfer_sld']);
            }
            ...
            if (!$tld instanceof \Model_Tld) {
                throw new \Priyx_Exception('TLD not found');
            }

            $domain = $data['transfer_sld'].$tld->tld;
            if (!$this->canBeTransfered($tld, $data['transfer_sld'])) {
                throw new \Priyx_Exception(':domain can not be transferred!', [':domain' => $domain]);
            }
        ...
        if ('register' == $action) {
            if (!isset($data['register_sld'])) {
                throw new \Priyx_Exception('Order data must contain :field configuration field', [':field' => 'register_sld']);
            }
            ...
            if ($years < $tld->min_years) {
                throw new \Priyx_Exception(':tld can be registered for at least :years years', [':tld' => $tld->tld, ':years' => $tld->min_years]);
            }

            $domain = $data['register_sld'].$tld->tld;
            if (!$this->isDomainAvailable($tld, $data['register_sld'])) {
                throw new \Priyx_Exception(':domain is already registered!', [':domain' => $domain]);
            }
    ...
    public function action_create(\Model_ClientOrder $order)
    {
        ...
        if (empty($ns)) {
            throw new \Priyx_Exception('Default domain nameservers are not configured');
        }
    ...
    public function action_activate(\Model_ClientOrder $order)
    {
        ...
        if (!$model instanceof \Model_ServiceDomain) {
            throw new \Priyx_Exception('Could not activate order. Service was not created');
        }
    ...
    {
        if (!isset($data['ns1'])) {
            throw new \Priyx_Exception('Nameserver 1 is required');
        }
        if (!isset($data['ns2'])) {
            throw new \Priyx_Exception('Nameserver 2 is required');
        }
    ...
    {
        if (empty($sld)) {
            throw new \Priyx_Exception('Domain name is not valid');
        }

        if (!$model->allow_transfer) {
            throw new \Priyx_Exception('Domain can not be transferred', null, 403);
        }
    ...
    public function isDomainAvailable(\Model_Tld $model, $sld)
    {
        ...
        if (!$model->allow_register) {
            throw new \Priyx_Exception('Domain can not be registered', null, 403);
        }
    ...
    public function registrarGetAvailable()
    {
        ...
        $pattern = PS_PATH_LIBRARY.'/Registrar/Adapter/*.php';
        $adapters = [];
    ...
    {
        if (!file_exists(PS_PATH_LIBRARY.'/Registrar/Adapter/'.$model->registrar.'.php')) {
            throw new \Priyx_Exception('Domain registrar :adapter was not found', [':adapter' => $model->registrar]);
        }

        $class = sprintf('Registrar_Adapter_%s', $model->registrar);
        if (!class_exists($class)) {
            throw new \Priyx_Exception('Registrar :adapter was not found', [':adapter' => $class]);
        }
    ...
    public function registrarGetRegistrarAdapter(\Model_TldRegistrar $r)
    {
        ...
        if (!$registrar instanceof \Registrar_AdapterAbstract) {
            throw new \Priyx_Exception('Registrar adapter :adapter should extend Registrar_AdapterAbstract', [':adapter' => $class]);
        }
    ...
    public function registrarRm(\Model_TldRegistrar $model)
    {
        ...
        if (count($domains) > 0) {
            throw new \Priyx_Exception('Can not remove registrar which has domains');
        }
