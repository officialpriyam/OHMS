...
namespace Priyx\Mod\Servicedomain\Api;
...
class Guest extends \Api_Abstract
{
    ...
    public function check($data)
    {
        ...
        if (!$validator->isSldValid($sld)) {
            throw new \Priyx_Exception('Domain :domain is not valid', [':domain' => $sld]);
        }

        $tld = $this->getService()->tldFindOneByTld($data['tld']);
        if (!$tld instanceof \Model_Tld) {
            throw new \Priyx_Exception('Domain availability could not be determined. TLD is not active.');
        }

        if (!$this->getService()->isDomainAvailable($tld, $sld)) {
            throw new \Priyx_Exception('Domain is not available.');
        }
    ...
    public function can_be_transferred($data)
    {
        ...
        $tld = $this->getService()->tldFindOneByTld($data['tld']);
        if (!$tld instanceof \Model_Tld) {
            throw new \Priyx_Exception('TLD is not active.');
        }
        if (!$this->getService()->canBeTransfered($tld, $data['sld'])) {
            throw new \Priyx_Exception('Domain can not be transferred.');
        }
