...
class Service
{
    ...
    {
        if (empty($param)) {
            throw new \Priyx_Exception('Parameter key is missing');
        }
    ...
    public function getMessages($type)
    {
        ...
        try {
            ...
            if ($updater->getCanUpdate()) {
                ...
                $msgs['info'][] = sprintf('OHMS %s is available for download.', $version);
            }
        } catch (\Exception $e) {
        ...
        $install = PS_PATH_ROOT.'/install';
        if ($this->di['tools']->fileExists(PS_PATH_ROOT.'/install')) {
            $msgs['info'][] = sprintf('Install module "%s" still exists. Please remove it for security reasons.', $install);
        }

        if ('0.0.1' == $this->getVersion()) {
            $msgs['info'][] = 'OHMS couldn\'t find valid version information. This is okay if you downloaded OHMS directly from the master branch, instead of a released version. But beware, the master branch may not be stable enough for production use.';
        }

        if (!extension_loaded('openssl')) {
            $msgs['info'][] = sprintf('OHMS requires %s extension to be enabled on this server for security reasons.', 'php openssl');
        }
    ...
    {
        $p = \Priyx_Period::getPredefined();
        if (isset($p[$code])) {
            return $p[$code];
        }

        $p = new \Priyx_Period($code);
    ...
    public function getPublicParamValue($param)
    {
        ...
        $results = $stmt->fetchColumn();
        if (false === $results) {
            throw new \Priyx_Exception('Parameter :param does not exist', [':param' => $param]);
        }
    ...
    public function getPhoneCodes($data)
    {
        ...
        if (isset($data['country'])) {
            if (array_key_exists($data['country'], $codes)) {
                ...
                throw new \Priyx_Exception('Country :code phone code is not registered', [':code' => $data['country']]);
            }
        }
