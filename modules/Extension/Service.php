...
class Service implements InjectionAwareInterface
{
    ...
    public static function onBeforeAdminCronRun(\Priyx_Event $event)
    {
        $di = $event->getDi();
    ...
    public function update(\Model_Extension $model)
    {
        ...
        if ('mod' == $model->type) {
            ...
            if (empty($latest)) {
                throw new \Priyx_Exception('Could not retrieve version information for extension :ext', [':ext' => $model->name], 745);
            }
            $haveUpdate = (version_compare($model->version, $latest) < 0);
            if (!$haveUpdate) {
                throw new \Priyx_Exception('Latest :mod version installed. No need to update', [':mod' => $model->name], 785);
            }

            $extension = $ext->getExtension($model->name);
            if (empty($extension)) {
                throw new \Priyx_Exception('Could not retrieve :ext information', [':ext' => $model->name], 744);
            }

            throw new \Priyx_Exception('Visit extension site for update information.', null, 252);
            $result = [
    ...
    {
        switch ($ext->type) {
            case \Priyx_Extension::TYPE_HOOK:
                $file = ucfirst($ext->name).'.php';
                $destination = PS_PATH_LIBRARY.'/Hook/'.$file;
            ...
            case \Priyx_Extension::TYPE_MOD:
                ...
                if ($this->isCoreModule($mod)) {
                    throw new \Priyx_Exception('OHMS core modules can not be managed');
                }
    ...
    public function downloadAndExtract($type, $id)
    {
        ...
        $extracted = PS_PATH_CACHE.'/'.md5(uniqid());
        $zip = PS_PATH_CACHE.'/'.md5(uniqid()).'.zip';
        ...
        if (true === $res) {
            ...
            throw new \Priyx_Exception('Could not extract extension zip file');
        }

        // install by type
        switch ($type) {
            case \Priyx_Extension::TYPE_MOD:
                $destination = PS_PATH_MODS.'/mod_'.$id;
                if ($this->di['tools']->fileExists($destination)) {
                    throw new \Priyx_Exception('Module already installed.', null, 436);
                }
                if (!$this->di['tools']->rename($extracted, $destination)) {
                    throw new \Priyx_Exception('Extension can not be moved. Make sure your server write permissions to modules folder.', null, 437);
                }
                break;

            case \Priyx_Extension::TYPE_THEME:
                $destination = PS_PATH_THEMES.'/'.$id;
                if (!$this->di['tools']->fileExists($destination)) {
                    if (!$this->di['tools']->rename($extracted, $destination)) {
                        throw new \Priyx_Exception('Extension can not be moved. Make sure your server write permissions to themes folder.', null, 439);
                    }
                }
                break;

            case \Priyx_Extension::TYPE_TRANSLATION:
                $destination = PS_PATH_LANGS.'/'.$id.'/LC_MESSAGES';
                $this->di['tools']->emptyFolder($destination);
                if (!$this->di['tools']->fileExists($destination)) {
                    $this->di['tools']->mkdir($destination, 0777, true);
                }
                if (!$this->di['tools']->rename($extracted, $destination)) {
                    throw new \Priyx_Exception('Extension can not be moved. Make sure your server write permissions to locale folder.', null, 440);
                }
                break;

            default:
                throw new \Priyx_Exception('Extension does not support auto-install feature. Extension must be installed manually');
        }
    ...
    private function installModule(\Model_Extension $ext)
    {
        ...
        if ($mod->isCore()) {
            throw new \Priyx_Exception('OHMS core modules can not be installed or removed');
        }

        $info = $mod->getManifest();
        if (isset($info['minimum_OHMS_version']) && Priyx_Version::compareVersion($info['minimum_OHMS_version']) > 0) {
            throw new \Priyx_Exception('Module can not be installed. It requires at least :min version of OHMS. You are using :v', [':min' => $info['minimum_OHMS_version'], ':v' => Priyx_Version::VERSION]);
        }
