...
namespace Priyx\Mod\Extension\Api;
...
class Admin extends \Api_Abstract
{
    ...
    /**
     * Get list of extensions from extensions.OHMS.org
     * which can be installed on current version of OHMS.
     *
    ...
    public function update_core($data)
    {
        ...
        if (!$updater->getCanUpdate()) {
            throw new \Priyx_Exception('You have latest version of OHMS. You do not need to update.', null, 930);
        }

        $new_version = $updater->getLatestVersion();
        $updater->performUpdate();

        $this->di['logger']->info('Updated OHMS from %s to %s', \Priyx_Version::VERSION, $new_version);
    ...
    /**
     * Completely remove extension from OHMS.
     *
     * @param string $type - extensions type: mod, theme, gateway ...
     * @param string $id   - extension id
     *
    ...
    /**
     * Universal method for OHMS extensions
    ...
    private function _getExtension($data)
    {
        ...
        if (!$ext instanceof \Model_Extension) {
            throw new \Priyx_Exception('Extension not found');
        }
