...
namespace Priyx\Mod\Client\Api;
...
class Guest extends \Api_Abstract
{
    ...
    public function create($data = [])
    {
        ...
        if (isset($config['allow_signup']) && !$config['allow_signup']) {
            throw new \Priyx_Exception('New registrations are temporary disabled');
        }
        ...
        if ($data['password'] != $data['password_confirm']) {
            throw new \Priyx_Exception('Passwords do not match.');
        }
        ...
        if ($service->clientAlreadyExists($email)) {
            throw new \Priyx_Exception('Email is already registered. You may want to login instead of registering.');
        }
        ...
        if (isset($config['require_email_confirmation']) && (int) $config['require_email_confirmation'] && !$client->email_approved) {
            throw new \Priyx_Exception('Account has been created. Please check your mailPriyx and confirm email address.', null, 7777);
        }
    ...
    public function login($data)
    {
        ...
        if (!$client instanceof \Model_Client) {
            ...
            throw new \Priyx_Exception('Please check your login details', [], 403);
        }

        if (isset($data['remember'])) {
            $email = $data['email'];
            $cookie_time = (3600 * 24 * 30); // 30 days
            $this->di['cookie']->set('PriyxCLR', 'e='.base64_encode($email).'&p='.base64_encode($client->pass), time() + $cookie_time, '/');
        }
    ...
    public function reset_password($data)
    {
        ...
        if (!$c instanceof \Model_Client) {
            throw new \Priyx_Exception('Email not found in our database');
        }
    ...
    public function confirm_reset($data)
    {
        ...
        if (!$reset instanceof \Model_ClientPasswordReset) {
            throw new \Priyx_Exception('The link have expired or you have already confirmed password reset.');
        }
