...
class Service implements InjectionAwareInterface
{
    ...
    public function approveClientEmailByHash($hash)
    {
        ...
        if (!$result) {
            throw new \Priyx_Exception('Invalid email confirmation link');
        }
    ...
    public function canChangeCurrency(\Model_Client $model, $currency = null)
    {
        ...
        if ($invoice instanceof \Model_Invoice) {
            throw new \Priyx_Exception('Currency can not be changed. Client already have invoices issued.');
        }

        $order = $this->di['db']->findOne('ClientOrder', 'client_id = :client_id', [':client_id' => $model->id]);
        if ($order instanceof \Model_ClientOrder) {
            throw new \Priyx_Exception('Currency can not be changed. Client already have orders.');
        }
    ...
    {
        if (!$client->currency) {
            throw new \Priyx_Exception('Define clients currency before adding funds.');
        }

        if (!is_numeric($amount)) {
            throw new \Priyx_Exception('Funds amount is not valid');
        }

        if (empty($description)) {
            throw new \Priyx_Exception('Funds description is not valid');
        }
    ...
    public function get($data)
    {
        if (!isset($data['id']) && !isset($data['email'])) {
            throw new \Priyx_Exception('Client ID or email is required');
        }
        ...
        if (!$client instanceof \Model_Client) {
            throw new \Priyx_Exception('Client not found');
        }
    ...
    public function deleteGroup(\Model_ClientGroup $model)
    {
        ...
        if ($client) {
            throw new \Priyx_Exception('Can not remove group with clients');
        }
    ...
    public function authorizeClient($email, $plainTextPassword)
    {
        ...
        if (isset($config['require_email_confirmation']) && (int) $config['require_email_confirmation']) {
            if (!$model->email_approved) {
                ...
                if (!is_null($meta)) {
                    throw new \Priyx_Exception('Please check your mailPriyx and confirm email address.');
                } else {
                    $this->sendEmailConfirmationForClient($model);
                    throw new \Priyx_Exception('Confirmation email was sent to your email address. Please click on link in it in order to verify your email.');
                }
            }
        }
    ...
    public function canChangeEmail(\Model_Client $client, $email)
    {
        ...
        if ($client->email != $email
            ...
            throw new \Priyx_Exception('Email can not be changed');
        }
    ...
    public function checkExtraRequiredFields(array $checkArr)
    {
        ...
        foreach ($required as $field) {
            if (!isset($checkArr[$field]) || empty($checkArr[$field])) {
                ...
                throw new \Priyx_Exception('It is required that you provide details for field ":field"', [':field' => $name]);
            }
        }
    }
