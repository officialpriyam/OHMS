...
namespace Priyx\Mod\Servicesolusvm\Api;
...
class Admin extends \Api_Abstract
{
    private function _getService($data)
    {
        if (!isset($data['order_id'])) {
            throw new \Priyx_Exception('Order id is required');
        }
        ...
        if (!$order) {
            throw new \Priyx_Exception('Solusvm order not found');
        }
    ...
    /**
     * Import selected servers to OHMS.
     *
    ...
    public function import_servers($data)
    {
        ...
        if (empty($nodeid)) {
            throw new \Priyx_Exception('Node is not selected for import.', null, 235);
        }

        if (empty($selected)) {
            throw new \Priyx_Exception('No servers selected for import.', null, 234);
        }

        if (empty($product_id)) {
            throw new \Priyx_Exception('Select product for orders.');
        }
        ...
        $this->di['logger']->info('Imported VPS from SolusVM to OHMS');

        return $log;
    }

    /**
     * Import selected clients to OHMS.
     *
     * @return string - log information regarding import process
     */
    public function import_clients($data)
    {
        ...
        if (empty($selected)) {
            throw new \Priyx_Exception('No clients selected for import.', null, 233);
        }

        $clients = $this->client_list($data);
        if (empty($clients)) {
            throw new \Priyx_Exception('No clients found on SolusVM server.');
        }
        ...
        $this->di['logger']->info('Imported clients from SolusVM to OHMS');
