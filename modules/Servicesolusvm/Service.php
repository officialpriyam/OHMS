...
namespace Priyx\Mod\Servicesolusvm;
...
class Service implements InjectionAwareInterface
{
    ...
    public function validateOrderData(array $data)
    {
        if (!isset($data['hostname']) || empty($data['hostname'])) {
            throw new \Priyx_Exception('Please enter VPS hostname.', null, 7101);
        }

        $ValidHostnameRegex = "/^(([a-zA-Z]|[a-zA-Z][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z]|[A-Za-z][A-Za-z0-9\-]*[A-Za-z0-9])$/";
        if (!preg_match($ValidHostnameRegex, $data['hostname'])) {
            throw new \Priyx_Exception('Hostname :hostname is not valid.', [':hostname' => $data['hostname']], 7102);
        }

        if (!isset($data['template']) || empty($data['template'])) {
            throw new \Priyx_Exception('Please select VPS template.', null, 7103);
        }
    }
    ...
    {
        if (!isset($data['id'])) {
            throw new \Priyx_Exception('SolusVM API ID is missing.', null, 7201);
        }

        if (!isset($data['key'])) {
            throw new \Priyx_Exception('SolusVM API KEY is missing.', null, 7202);
        }

        if (!isset($data['ipaddress'])) {
            throw new \Priyx_Exception('SolusVM API ipaddress is missing.', null, 7203);
        }

        if (!isset($data['secure'])) {
            throw new \Priyx_Exception('SolusVM API secure is missing.', null, 7204);
        }

        if (!isset($data['port'])) {
            throw new \Priyx_Exception('SolusVM API port is missing.', null, 7204);
        }
    ...
    public function getMasterConfig($cluster_id)
    {
        ...
        if (!$config) {
            ...
            $config = json_decode((string) $config, 1);
        }
    ...
    {
        if (!is_object($model)) {
            throw new \Priyx_Exception('Could not activate order. Service was not created', null, 7456);
        }

        $client = $this->di['db']->load('client', $order->client_id);
        $product = $this->di['db']->load('product', $order->product_id);
        if (!$product) {
            throw new \Priyx_Exception('Could not activate order because ordered product does not exists', null, 7457);
        }

        $pconfig = json_decode((string) $product->config, 1);
        ...
        if (!isset($pconfig['node'])) {
            throw new \Priyx_Exception('Could not activate order. Product :id configuration is missing node parameter', [':id' => $product->id], 7458);
        }

        if (!isset($pconfig['plan'])) {
            throw new \Priyx_Exception('Could not activate order. Product :id configuration is missing plan parameter', [':id' => $product->id], 7459);
        }

        if (!isset($pconfig['vtype'])) {
            throw new \Priyx_Exception('Could not activate order. Product :id configuration is missing virtualization type parameter', [':id' => $product->id], 7460);
        }

        if (!isset($pconfig['ips'])) {
            throw new \Priyx_Exception('Could not activate order. Product :id configuration is missing ips amount parameter', [':id' => $product->id], 7461);
        }
        ...
        $model->cluster_id = 1; // for future if ever OHMS supports multiple master servers
    ...
    public function set_root_password($order, $model, $params = [])
    {
        if (!isset($params['password']) || strlen($params['password']) < 4) {
            throw new \Priyx_Exception('Root password must be longer than 4 symbols', null, 8789);
        }
    ...
    public function set_plan($order, $model, $params = [])
    {
        if (!isset($params['plan']) || empty($params['plan'])) {
            throw new \Priyx_Exception('Plan name must not be empty', null, 8790);
        }
    ...
    public function set_hostname($order, $model, $params = [])
    {
        if (!isset($params['hostname']) || empty($params['hostname'])) {
            throw new \Priyx_Exception('Hostname must not be empty', null, 8791);
        }
    ...
    public function rebuild($order, $model, $params = [])
    {
        if (!isset($params['template']) || empty($params['template'])) {
            throw new \Priyx_Exception('Template must not be empty', null, 8792);
        }
    ...
    {
        if (!isset($params['password']) || strlen($params['password']) < 4) {
            throw new \Priyx_Exception('Password must be longer than 4 symbols', null, 8790);
        }
