...
class Admin extends \Api_Abstract
{
    ...
    public function message_add($data)
    {
        ...
        if (!method_exists($service, $handler)) {
            throw new \Priyx_Exception('Message handler function :method does not exists', [':ext' => $data['mod'], ':method' => get_class($service).':'.$handler]);
        }
    ...
    public function execute($data)
    {
        ...
        $lock_file = PS_PATH_LOG.'/queue_'.$q->id.'.lock';
        touch($lock_file);
