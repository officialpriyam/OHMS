...
namespace Priyx\Mod\Spamchecker;
...
class Service implements InjectionAwareInterface
{
    ...
    public function forumSpamChecker(\Priyx_Event $event)
    {
        $di = $event->getDi();
    ...
    public static function onBeforeClientCreateForumTopic(\Priyx_Event $event)
    {
        $di = $event->getDi();
        $spamCheckerService = $di['mod_service']('Spamchecker');
    ...
    public static function onBeforeClientRepliedInForum(\Priyx_Event $event)
    {
        $di = $event->getDi();
        $spamCheckerService = $di['mod_service']('Spamchecker');
        $spamCheckerService->forumSpamChecker($event);
    ...
    public function isBlockedIp($event)
    {
        ...
        if (isset($config['block_ips']) && $config['block_ips'] && isset($config['blocked_ips'])) {
            ...
            if (in_array($di['request']->getClientAddress(), $blocked_ips)) {
                throw new \Priyx_Exception('Your IP addresss (:ip) is blocked. Please contact our support to lift your block.', [':ip' => $di['request']->getClientAddress()], 403);
            }
        }
    }
    ...
    public function isCommentSpam($event, $comment)
    {
        ...
        require_once PS_PATH_MODS.'/Spamchecker/akismet.curl.class.php';
        ...
        if ($akismet->is_spam($comment)) {
            throw new \Priyx_Exception('Akismet detected this message is spam');
        }
    }

    public function isSpam(\Priyx_Event $event)
    {
        $di = $event->getDi();
        $params = $event->getParameters();
        $data = [
        ...
        if (isset($config['captcha_enabled']) && $config['captcha_enabled']) {
            if (isset($config['captcha_version']) && 2 == $config['captcha_version']) {
                if (!isset($config['captcha_recaptcha_privatekey']) || '' == $config['captcha_recaptcha_privatekey']) {
                    throw new \Priyx_Exception("To use reCAPTCHA you must get an API key from <a href='https://www.google.com/recaptcha/admin/create'>here</a>");
                }

                if (!isset($params['g-recaptcha-response']) || '' == $params['g-recaptcha-response']) {
                    throw new \Priyx_Exception('You have to complete the CAPTCHA to continue');
                }
                ...
                if (!$content['success']) {
                    throw new \Priyx_Exception('reCAPTCHA verification failed.');
                }
            } else {
                throw new \Priyx_Exception('reCAPTCHA verification failed.');
            }
    ...
    public function isInStopForumSpamDatabase(array $data)
    {
        ...
        if (isset($json->username->appears) && $json->username->appears) {
            throw new \Priyx_Exception('Your username is blacklisted in the Stop Forum Spam database');
        }
        if (isset($json->email->appears) && $json->email->appears) {
            throw new \Priyx_Exception('Your e-mail is blacklisted in the Stop Forum Spam database');
        }
        if (isset($json->ip->appears) && $json->ip->appears) {
            throw new \Priyx_Exception('Your IP address is blacklisted in the Stop Forum Spam database');
        }
