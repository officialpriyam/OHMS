...
namespace Priyx\Mod\Servicelicense;
...
class Service implements InjectionAwareInterface
{
    ...
    public function action_activate(\Model_ClientOrder $order)
    {
        ...
        if (!is_object($plugin)) {
            throw new \Priyx_Exception('License plugin :plugin was not found', [':plugin' => $model->plugin]);
        }

        if (!method_exists($plugin, 'generate')) {
            throw new \Priyx_Exception('License plugin do not have generate method');
        }
        ...
        do {
            ...
            if ($i++ >= $iterations) {
                throw new \Priyx_Exception('Maximum number of iterations reached while generating license key');
            }
    ...
    private function _getPlugin(\Model_ServiceLicense $model)
    {
        ...
        foreach ($plugins as $plugin) {
            if ($model->plugin == $plugin['filename']) {
                ...
                $class_name = 'Priyx\\Mod\\Servicelicense\\Plugin\\'.$model->plugin;
    ...
    {
        $log = $this->di['logger'];
        $log->addWriter(new \Priyx_LogStream(PS_PATH_LOG.'/license.log'));
         ...
         * If error comes from OHMS core use $result['error'] field.
         *
         * @since v2.7.1
         */
