...
namespace Priyx\Mod\Servicehosting;
...
class Service implements InjectionAwareInterface
{
    ...
    public function validateOrderData(array &$data)
    {
        if (!isset($data['server_id'])) {
            throw new \Priyx_Exception('Hosting product is not configured completely. Configure server for hosting product.', null, 701);
        }
        if (!isset($data['hosting_plan_id'])) {
            throw new \Priyx_Exception('Hosting product is not configured completely. Configure hosting plan for hosting product.', null, 702);
        }
        if (!isset($data['sld']) || empty($data['sld'])) {
            throw new \Priyx_Exception('Domain name is not valid.', null, 703);
        }
        if (!isset($data['tld']) || empty($data['tld'])) {
            throw new \Priyx_Exception('Domain extension is not valid.', null, 704);
        }
    }
    ...
    public function changeAccountUsername(\Model_ClientOrder $order, \Model_ServiceHosting $model, $data)
    {
        if (!isset($data['username']) || empty($data['username'])) {
            throw new \Priyx_Exception('Account username is missing or is not valid');
        }
    ...
    public function changeAccountIp(\Model_ClientOrder $order, \Model_ServiceHosting $model, $data)
    {
        if (!isset($data['ip']) || empty($data['ip'])) {
            throw new \Priyx_Exception('Account ip is missing or is not valid');
        }
    ...
    public function changeAccountDomain(\Model_ClientOrder $order, \Model_ServiceHosting $model, $data)
    {
        if (!isset($data['tld']) || empty($data['tld']) ||
           !isset($data['sld']) || empty($data['sld'])) {
            throw new \Priyx_Exception('Domain sld or tld is missing');
        }
    ...
    public function changeAccountPassword(\Model_ClientOrder $order, \Model_ServiceHosting $model, $data)
    {
        if (!isset($data['password']) || !isset($data['password_confirm'])
            ...
            throw new \Priyx_Exception('Account password is missing or is not valid');
        }
    ...
    {
        $dir = PS_PATH_LIBRARY . '/Server/Manager';
        $files = [];
        ...
        // Scan modules for managers
        $handle = opendir(PS_PATH_MODS);
        while ($name = readdir($handle)) {
            if (ctype_alnum($name)) {
                $managerName = ucfirst($name);
                $modDir = PS_PATH_MODS . '/' . $name . '/Lib/Server/Manager';
                if (is_dir($modDir)) {
                    $managerHandle = opendir($modDir);
                    while ($managerItem = readdir($managerHandle)) {
                        if (('.' != $managerItem) && ('..' != $managerItem) && ('.php' == substr($managerItem, -4))) {
                            $managerCode = pathinfo($managerItem, PATHINFO_FILENAME);
                            if (!in_array($managerCode, $files)) {
                                $files[] = $managerCode;
                            }
                        }
                    }
                    closedir($managerHandle);
                }
            }
        }
        closedir($handle);
    ...
    {
        $filename = PS_PATH_LIBRARY . '/Server/Manager/' . $manager . '.php';
        $classname = 'Server_Manager_' . $manager;

        if (!file_exists($filename)) {
            // Try to find in modules
            $managerName = ucfirst($manager);
            $classname = sprintf('\Priyx\Mod\%s\Lib\Server\Manager\%s', $managerName, $managerName);
    ...
    public function getServerManager(\Model_ServiceHostingServer $model)
    {
        if (empty($model->manager)) {
            throw new \Priyx_Exception('Invalid server manager. Server was not configured properly.', null, 654);
        }
        ...
        if (!$manager instanceof \Server_Manager) {
            throw new \Priyx_Exception('Server manager :adapter is not valid', [':adapter' => $model->manager]);
        }
    ...
    public function deleteHp(\Model_ServiceHostingHp $model)
    {
        ...
        if ($serviceHosting) {
            throw new \Priyx_Exception('Can not remove hosting plan which has active accounts');
        }
        $this->di['db']->trash($model);
    ...
    public function getDomainProductFromConfig(\Model_Product $product, array &$data)
    {
        ...
        if (!$d instanceof \Model_Product) {
            throw new \Priyx_Exception('Could not find main domain product');
        }
