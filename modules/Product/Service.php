...
class Service implements InjectionAwareInterface
{
     ...
     */
    protected $di = null;
    protected ?ConfigurableOptionsManager $configurableOptionsManager = null;
    private bool $categorySchemaChecked = false;
    ...
    protected function getConfigurableOptionsManager(): ConfigurableOptionsManager
    {
        if (!$this->configurableOptionsManager instanceof ConfigurableOptionsManager) {
            $this->configurableOptionsManager = new ConfigurableOptionsManager();
            $this->configurableOptionsManager->setDi($this->di);
        }

        $this->configurableOptionsManager->ensureSchema();

        return $this->configurableOptionsManager;
    }

    public function install(): void
    {
        $this->ensureProductCategorySchema();
        $this->getConfigurableOptionsManager()->ensureSchema();
    }

    public function update($manifest = null): void
    {
        $this->ensureProductCategorySchema();
        $this->getConfigurableOptionsManager()->ensureSchema();
    }
    ...
    public function toApiArray(\Model_Product $model, $deep = true, $identity = null)
    {
        ...
        $configurableOptions = $deep
            ? $this->getConfigurableOptionsManager()->getProductGroupsApiArray($model, true, $identity instanceof \Model_Admin)
            : [];
        ...
        if ($model->product_category_id) {
            ...
            if ($productCategory instanceof \Model_ProductCategory) {
                $this->lazyEnsureCategorySchema();
                $themeService = $this->di['mod_service']('theme');
                $result['category'] = [
                    'id' => $productCategory->id,
                    'title' => $productCategory->title,
                    'orderform_code' => $productCategory->orderform_code ?: null,
                    'effective_orderform_code' => $themeService->getEffectiveOrderformCode($productCategory),
                ];
                $result['effective_orderform_code'] = $result['category']['effective_orderform_code'];
            }
        }
    ...
    public function getTypes()
    {
        ...
        // Specifically check for Pterodactyl and other logic-providing modules
        if ($extensionService->isExtensionActive('mod', 'Pterodactyl')) {
            $data['pterodactyl'] = 'Pterodactyl';
        }
    ...
    public function updateProduct(\Model_Product $model, $data)
    {
        ...
        if (isset($data['pricing'])) {
            ...
            if (!isset($data['pricing']['type']) || !array_key_exists($data['pricing']['type'], $types)) {
                throw new \Priyx_Exception('Pricing type is required');
            }
        ...
        if (array_key_exists('configurable_option_groups', $data)) {
            $groupIds = is_array($data['configurable_option_groups']) ? $data['configurable_option_groups'] : [];
            $this->getConfigurableOptionsManager()->syncProductGroups($model, $groupIds);
        }
    ...
    public function deleteProduct(\Model_Product $product)
    {
        ...
        if ($orderService->productHasOrders($product)) {
            throw new \Priyx_Exception('Can not remove product which has active orders.');
        }
        $id = $product->id;
    ...
    public function updateCategory(\Model_ProductCategory $productCategory, $title = null, $description = null, $icon_url = null, $orderformCode = null)
    {
        $this->lazyEnsureCategorySchema();
        $productCategory->title = $title;
        $productCategory->icon_url = $icon_url;
        $productCategory->description = $description;
        $productCategory->orderform_code = $this->normalizeOrderformCode($orderformCode);
    ...
    public function createCategory($title, $description = null, $icon_url = null, $orderformCode = null)
    {
        $this->lazyEnsureCategorySchema();
        $systemService = $this->di['mod_service']('system');
        ...
        $model->orderform_code = $this->normalizeOrderformCode($orderformCode);
        $model->updated_at = date('Y-m-d H:i:s');
        $model->created_at = date('Y-m-d H:i:s');
    ...
    public function removeProductCategory(\Model_ProductCategory $category)
    {
        ...
        if ($model instanceof \Model_Product) {
            throw new \Priyx_Exception('Can not remove product category with products');
        }
        $id = $category->id;
    ...
    public function createPromo($code, $type, $value, $products, $periods, $clientGroups, $data)
    {
        if ($this->di['db']->findOne('Promo', 'code = :code', [':code' => $code])) {
            throw new \Priyx_Exception('This promo code already exists.');
        }
    ...
    public function toProductCategoryApiArray(\Model_ProductCategory $model, $deep = true, $identity = null)
    {
        ...
        $themeService = $this->di['mod_service']('theme');
        $data['orderform_code'] = $model->orderform_code ?: null;
        $data['effective_orderform_code'] = $themeService->getEffectiveOrderformCode($model);
    ...
    protected function normalizeOrderformCode($orderformCode): ?string
    {
        $orderformCode = trim((string) $orderformCode);

        return $orderformCode === '' ? null : $orderformCode;
    }

    protected function lazyEnsureCategorySchema(): void
    {
        if ($this->categorySchemaChecked) {
            return;
        }
        $this->categorySchemaChecked = true;
        $this->ensureProductCategorySchema();
    }

    protected function ensureProductCategorySchema(): void
    {
        try {
            $rows = $this->di['db']->getAll('DESCRIBE `product_category`');
            $exists = false;
            foreach ($rows as $row) {
                if ($row['Field'] == 'orderform_code' || $row['Field'] == '`orderform_code`') {
                    $exists = true;
                    break;
                }
            }
            ...
            if (!$exists) {
                $this->di['db']->exec("ALTER TABLE `product_category` ADD COLUMN `orderform_code` varchar(64) DEFAULT NULL");
            }
        } catch (\Exception $e) {
    ...
    public function getStartingFromPrice(\Model_Product $model)
    {
        ...
        if ($model->product_payment_id) {
            ...
            $manager = $this->getConfigurableOptionsManager();

            if ('recurrent' == $productPaymentModel->type) {
                $basePricing = $this->toProductPaymentApiArray($productPaymentModel);
                $prices = [];
                foreach ($basePricing['recurrent'] as $period => $details) {
                    if (!$details['enabled']) {
                        continue;
                    }

                    $prices[] = (float) $details['price'] + $manager->getMinimumRequiredPrice($model, $period);
                }

                return empty($prices) ? 0.0 : min($prices);
            }

            return $this->getStartingPrice($productPaymentModel) + $manager->getMinimumRequiredPrice($model);
        }

        return null;
    }

    public function getConfigurableOptionTypes(): array
    {
        return $this->getConfigurableOptionsManager()->getOptionTypes();
    }

    public function getConfigurableOptionPricingPeriods(): array
    {
        return $this->getConfigurableOptionsManager()->getPricingPeriods();
    }

    public function getConfigurableGroupList(bool $deep = true, bool $admin = true): array
    {
        return $this->getConfigurableOptionsManager()->getGroupsApiArray($deep, $admin);
    }

    public function getConfigurableGroup(\Model_ProductConfigOptionGroup $group, bool $deep = true, bool $admin = true): array
    {
        return $this->getConfigurableOptionsManager()->toGroupApiArray($group, $deep, $admin);
    }

    public function createConfigurableGroup(array $data): int
    {
        return $this->getConfigurableOptionsManager()->createGroup($data);
    }

    public function updateConfigurableGroup(\Model_ProductConfigOptionGroup $group, array $data): bool
    {
        return $this->getConfigurableOptionsManager()->updateGroup($group, $data);
    }

    public function deleteConfigurableGroup(\Model_ProductConfigOptionGroup $group): bool
    {
        return $this->getConfigurableOptionsManager()->deleteGroup($group);
    }

    public function getConfigurableOption(\Model_ProductConfigOption $option, bool $deep = true, bool $admin = true): array
    {
        return $this->getConfigurableOptionsManager()->toOptionApiArray($option, $deep, $admin);
    }

    public function createConfigurableOption(\Model_ProductConfigOptionGroup $group, array $data): int
    {
        return $this->getConfigurableOptionsManager()->createOption($group, $data);
    }

    public function updateConfigurableOption(\Model_ProductConfigOption $option, array $data): bool
    {
        return $this->getConfigurableOptionsManager()->updateOption($option, $data);
    }

    public function deleteConfigurableOption(\Model_ProductConfigOption $option): bool
    {
        return $this->getConfigurableOptionsManager()->deleteOption($option);
    }

    public function getAssignedConfigurableGroupIds(\Model_Product $product): array
    {
        return $this->getConfigurableOptionsManager()->getAssignedGroupIds($product);
    }

    public function validateConfigurableSelections(\Model_Product $product, array $config): bool
    {
        return $this->getConfigurableOptionsManager()->validateSelections($product, $config);
    }

    public function getConfigurableSelectionSummary(\Model_Product $product, ?array $config = null, bool $validate = false): array
    {
        return $this->getConfigurableOptionsManager()->getSelectionSummary($product, $config, $validate);
    }

    public function getUpgradablePairs(\Model_Product $model)
    {
        $upgrades = trim((string) $model->upgrades);
        $ids = $upgrades !== '' ? (json_decode($upgrades, true) ?: []) : [];
    ...
    public function getStoredFileIdentifier($config)
    {
        if (!is_array($config)) {
            return null;
        }

        $storageName = $config['storage_name'] ?? null;
        if (is_string($storageName) && '' !== trim($storageName)) {
            return $storageName;
        }
    ...
    {
        $identifier = $this->getStoredFileIdentifier($config);
        if (null === $identifier) {
            return false;
        }

        $f = $this->getSavePath($identifier);
