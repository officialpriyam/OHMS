...
namespace Priyx\Mod\Product\Api;
...
class Admin extends \Api_Abstract
{
    ...
    public function prepare($data)
    {
        ...
        if ('domain' == $data['type']) {
            ...
            if ($model instanceof \Model_Product) {
                throw new \Priyx_Exception('You have already created domain product.', null, 413);
            }
        }

        $types = $service->getTypes();
        if (!array_key_exists($data['type'], $types)) {
            throw new \Priyx_Exception('Product type :type is not registered', [':type' => $data['type']], 413);
        }
    ...
    public function update_priority($data)
    {
        if (!isset($data['priority']) || !is_array($data['priority'])) {
            throw new \Priyx_Exception('priority params is missing');
        }
    ...
    public function addon_get($data)
    {
        ...
        if (!$model instanceof \Model_Product || !$model->is_addon) {
            throw new \Priyx_Exception('Addon not found');
        }
        $service = $this->getService();
    ...
    public function category_update($data)
    {
        ...
        $orderformCode = $this->di['array_get']($data, 'orderform_code', null);

        $service = $this->getService();

        return $service->updateCategory($model, $title, $description, $icon_url, $orderformCode);
    }
    ...
    public function category_create($data)
    {
        ...
        return (int) $service->createCategory($title, $description, $icon_url, $orderformCode);
    }
    ...
    public function configurable_group_get_list($data)
    {
        return $this->getService()->getConfigurableGroupList(true, true);
    }

    public function configurable_group_get($data)
    {
        $group = $this->_getConfigurableGroup($data);

        return $this->getService()->getConfigurableGroup($group, true, true);
    }

    public function configurable_group_create($data)
    {
        return $this->getService()->createConfigurableGroup($data);
    }

    public function configurable_group_update($data)
    {
        $group = $this->_getConfigurableGroup($data);

        return $this->getService()->updateConfigurableGroup($group, $data);
    }

    public function configurable_group_delete($data)
    {
        $group = $this->_getConfigurableGroup($data);

        return $this->getService()->deleteConfigurableGroup($group);
    }

    public function configurable_option_get($data)
    {
        $option = $this->_getConfigurableOption($data);

        return $this->getService()->getConfigurableOption($option, true, true);
    }

    public function configurable_option_create($data)
    {
        $required = [
            'group_id' => 'Configurable option group ID is required',
        ];
        $this->di['validator']->checkRequiredParamsForArray($required, $data);

        $group = $this->di['db']->getExistingModelById(
            'ProductConfigOptionGroup',
            $data['group_id'],
            'Configurable option group not found'
        );

        return $this->getService()->createConfigurableOption($group, $data);
    }

    public function configurable_option_update($data)
    {
        $option = $this->_getConfigurableOption($data);

        return $this->getService()->updateConfigurableOption($option, $data);
    }

    public function configurable_option_delete($data)
    {
        $option = $this->_getConfigurableOption($data);

        return $this->getService()->deleteConfigurableOption($option);
    }

    public function configurable_option_types($data)
    {
        return $this->getService()->getConfigurableOptionTypes();
    }

    public function configurable_option_pricing_periods($data)
    {
        return $this->getService()->getConfigurableOptionPricingPeriods();
    }
    ...
    private function _getConfigurableGroup($data)
    {
        $required = [
            'id' => 'Configurable option group ID is required',
        ];
        $this->di['validator']->checkRequiredParamsForArray($required, $data);
    ...
    private function _getConfigurableOption($data)
    {
        $required = [
            'id' => 'Configurable option ID is required',
        ];
        $this->di['validator']->checkRequiredParamsForArray($required, $data);
