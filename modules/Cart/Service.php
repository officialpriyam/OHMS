...
class Service implements InjectionAwareInterface
{
    ...
    public function addItem(\Model_Cart $cart, \Model_Product $product, array $data)
    {
        ...
        if ($this->isRecurrentPricing($product)) {
            ...
            if (!$this->isPeriodEnabledForProduct($product, $data['period'])) {
                throw new \Priyx_Exception('Selected billing period is not valid');
            }
        }
        ...
        if (!$this->isStockAvailable($product, $qty)) {
            throw new \Priyx_Exception("I'm afraid we are out of stock.");
        }
                ...
                if ($addon instanceof \Model_Product) {
                    if ($this->isRecurrentPricing($addon)) {
                        ...
                        if (!$this->isPeriodEnabledForProduct($addon, $ac['period'])) {
                            throw new \Priyx_Exception('Selected billing period is not valid for addon');
                        }
                    }
        ...
        foreach ($list as $c) {
            ...
            $productService->validateConfigurableSelections($productFromList, $productFromListConfig);
    ...
    public function removeProduct(\Model_Cart $cart, $id, $removeAddons = true)
    {
        ...
        if (!$cartProduct instanceof \Model_CartProduct) {
            throw new \Priyx_Exception('Product not found');
        }
    ...
    public function applyPromo(\Model_Cart $cart, \Model_Promo $promo)
    {
        ...
        if ($this->isEmptyCart($cart)) {
            throw new \Priyx_Exception('Add products to cart before applying promo code');
        }
    ...
    public function toApiArray(\Model_Cart $model, $deep = false, $identity = null)
    {
        ...
        foreach ($products as $product) {
            try {
                ...
                // Skip stale cart lines (e.g. deleted products) instead of hard-failing checkout/cart pages.
    ...
    private function getCartProductModel(\Model_CartProduct $cartProduct): \Model_Product
    {
        $product = $this->di['db']->load('Product', $cartProduct->product_id);
        if (!$product instanceof \Model_Product || empty($product->id)) {
            $missingId = (int) $cartProduct->product_id;
            $this->di['db']->trash($cartProduct);
            throw new \Priyx_Exception("Product #{$missingId} no longer exists and was removed from your cart.");
        }
    ...
    public function checkoutCart(\Model_Cart $cart, \Model_Client $client, $gateway_id = null)
    {
        if ($cart->promo_id) {
            ...
            if (!$this->isClientAbleToUsePromo($client, $promo)) {
                throw new \Priyx_Exception('You have already used this promo code. Please remove promo code and checkout again.', null, 9874);
            }
        }
    ...
    public function createFromCart(\Model_Client $client, $gateway_id = null)
    {
        ...
        if (0 == count($ca['items'])) {
            throw new \Priyx_Exception('Can not checkout empty cart.');
        }
        ...
        if ($client->currency != $currency->code) {
            throw new \Priyx_Exception('Selected currency :selected does not match your profile currency :code. Please change cart currency to continue.', [':selected' => $currency->code, ':code' => $client->currency]);
        }
        ...
        foreach ($this->getCartProducts($cart) as $p) {
             ...
             * https://github.com/OHMS/OHMS/discussions/1022#discussioncomment-1311819
             */
            if (isset($item['register_sld'])) { $item['register_sld'] = strtolower($item['register_sld']); }
            if (isset($item['transfer_sld'])) { $item['transfer_sld'] = strtolower($item['transfer_sld']); }
            if (isset($item['sld'])) { $item['sld'] = strtolower($item['sld']); }
            if (isset($item['domain']['owndomain_sld'])) { $item['domain']['owndomain_sld'] = strtolower($item['domain']['owndomain_sld']); }
            if (isset($item['domain']['register_sld'])) { $item['domain']['register_sld'] = strtolower($item['domain']['register_sld']); }
            if (isset($item['domain']['transfer_sld'])) { $item['domain']['transfer_sld'] = strtolower($item['domain']['transfer_sld']); }
    ...
    {
        $product = $this->getCartProductModel($model);
    ...
    public function cartProductToApiArray(\Model_CartProduct $model)
    {
        ...
        $config = $this->getItemConfig($model);
        $productService = $this->di['mod_service']('product');
        $configurableOptions = $productService->getConfigurableSelectionSummary($product, $config, false);
        ...
        $data = array_merge($config, [
            ...
            'total' => $subtotal,
            'config_options_data' => $configurableOptions['items'],
            'config_options_price' => $configurableOptions['price'],
            'period' => $this->di['array_get']($config, 'period'),
        ]);
