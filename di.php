...
$di['config'] = function () {
    $array = include PS_PATH_ROOT.'/config.php';

    return new Priyx_Config($array);
};
...
$di['logger'] = function () use ($di) {
    ...
    if ($log_to_db) {
        ...
        $writer2 = new Priyx_LogDb($activity_service);
        ...
        $writer = new Priyx_LogStream($logFile);
...
$di['pdo'] = function () use ($di) {
    ...
    if (isset($c['debug']) && $c['debug']) {
        $pdo->setAttribute(PDO::ATTR_STATEMENT_CLASS, ['Priyx_DbLoggedPDOStatement']);
    }
...
$di['url'] = function () use ($di) {
    $url = new Priyx_Url();
    $url->setDi($di);
    $url->setBaseUri(PS_URL);

    return $url;
};

$di['mod'] = $di->protect(function ($name) use ($di) {
    $mod = new Priyx_Mod($name);
    $mod->setDi($di);
...
$di['events_manager'] = function () use ($di) {
    $service = new Priyx_EventManager();
    $service->setDi($di);
...
$di['session'] = function () use ($di) {
    ...
    return new Priyx_Session($handler);
};

$di['cookie'] = function () use ($di) {
    $service = new Priyx_Cookie();
    $service->setDi($di);
...
$di['auth'] = function () use ($di) { return new Priyx_Authorization($di); };
...
$di['twig'] = $di->factory(function () use ($di) {
    ...
    $Priyx_extensions = new Priyx_TwigExtensions();
    $Priyx_extensions->setDi($di);

    // $twig->addExtension(new Twig\Extension\OptimizerExtension());
    $twig->addExtension(new \Twig\Extension\StringLoaderExtension());
    $twig->addExtension(new Twig\Extension\DebugExtension());
    $twig->addExtension(new Symfony\Bridge\Twig\Extension\TranslationExtension());
    $twig->addExtension($Priyx_extensions);
...
$di['mail'] = function () {
    return new Priyx_Mail();
};

$di['extension'] = function () use ($di) {
    $extension = new \Priyx_Extension();
    $extension->setDi($di);
...
$di['server_manager'] = $di->protect(function ($manager, $config) use ($di) {
    ...
    $moduleClass = sprintf('\Priyx\Mod\%s\Lib\Server\Manager\%s', $managerName, $managerName);
    if (class_exists($moduleClass)) {
        return new $moduleClass($config);
    }

    throw new \Exception(sprintf('Server manager "%s" was not found', $manager));
});

$di['requirements'] = function () use ($di) {
    $r = new Priyx_Requirements();
    $r->setDi($di);

    return $r;
};

$di['period'] = $di->protect(function ($code) { return new \Priyx_Period($code); });
...
$di['license_server'] = function () use ($di) {
    $server = new \Priyx\Mod\Servicelicense\Server($di['logger']);
    $server->setDi($di);
...
$di['solusvm'] = $di->protect(function ($config) use ($di) {
    $solusVM = new \Priyx\Mod\Servicesolusvm\SolusVM();
    $solusVM->setDi($di);
...
$di['service_OHMS'] = $di->protect(function ($config) {
    $service = new \Priyx\Mod\ServiceOHMSlicense\ServiceOHMS($config);

    return $service;
});

$di['ftp'] = $di->protect(function ($params) { return new \Priyx_Ftp($params); });

$di['pdf'] = function () use ($di) {
    include PS_PATH_LIBRARY.'/PDF_ImageAlpha.php';

    return new \PDF_ImageAlpha();
};

$di['geoip'] = function () { return new \GeoIp2\Database\Reader(PS_PATH_LIBRARY.'/GeoLite2-Country.mmdb'); };

$di['password'] = function () { return new Priyx_Password(); };

$di['translate'] = $di->protect(function ($textDomain = '') use ($di) {
    $tr = new Priyx_Translate();
