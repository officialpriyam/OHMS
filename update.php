 ...
 * 4.22 betas.
 */
class BBPatch_26 extends BBPatchAbstract
{
    public function patch()
    {
        // Add orderform_code column to product_category if missing
        try {
            $rows = $this->pdo->query('DESCRIBE `product_category`')->fetchAll(\PDO::FETCH_ASSOC);
            $exists = false;
            foreach ($rows as $row) {
                if ($row['Field'] === 'orderform_code') {
                    $exists = true;
                    break;
                }
            }
            if (!$exists) {
                $this->execSql("ALTER TABLE `product_category` ADD COLUMN `orderform_code` varchar(64) DEFAULT NULL");
            }
        } catch (\Exception $e) {
            error_log('BBPatch_26: ' . $e->getMessage());
        }
    }
}
...
class BBPatch_25 extends BBPatchAbstract
{
    public function patch()
    {
        $q = "CREATE TABLE IF NOT EXISTS `service_pterodactyl` (
          `id` bigint(20) NOT NULL AUTO_INCREMENT,
          `client_id` bigint(20) DEFAULT NULL,
          `pterodactyl_server_id` varchar(255) DEFAULT NULL,
          `pterodactyl_user_id` varchar(255) DEFAULT NULL,
          `config` text,
...
error_log('Executing OHMS update script');
natsort($patches);
foreach ($patches as $class) {
    $p = new $class($di);
    if (!$p->isPatched()) {
        $msg = 'OHMS patch #'.$p->getVersion().' executing...';
        error_log($msg);
        $p->patch();
        $p->donePatching();
        $msg = 'OHMS patch #'.$p->getVersion().' was executed';
        error_log($msg);
        echo $msg.PHP_EOL;
    } else {
...
error_log('OHMS update completed');

echo 'Update completed. You are using OHMS '.Priyx_Version::VERSION.PHP_EOL;
